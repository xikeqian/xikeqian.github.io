...
function() {
	...
	var t = {
		...
		BindEvent: function() {
			$(document).ajaxSuccess(function(e, i, o) {
				...
				i && $("#yxShopImg").css("background-image", "url(" + i + ")"), $(this).addClass("sd-option-02").siblings().removeClass("sd-option-02");
				var n = "",
					s = "",
					a = [];
				$(".sd-tk-option-infor-ul .sd-option-all a.sd-option-02").each(function(e, t) {
					var i = $(t).parent().prev("li").html();
					n += i + ":" + $(t).html() + ",", a.push($(t).data("id"))
				}), n = n.substring(0, n.length - 1), $("#yxShopChoose").html("已选：" + n), s = a.sort().join("_") + "_";
				for (var d = 0; d < t.goodDetail.invetory_details.length; d++) {
					var r = t.goodDetail.invetory_details[d];
					if (r.ids == s) {
						$("#yxShopPrice").html(o > r.inventory ? r.price : (r.price * o).toFixed(2)), $("#yxShopInventory").html("库存：" + r.inventory), $("#i_byu_num").html(o > r.inventory ? 1 : o);
						break
					}
				}
				t.ids = s
			}), $(document).on(ClickOrTap, "#addToCar", function(e) {
				...
				var i = parseInt($("#i_byu_num").html());
				$("#loadingToast").show(), t.ajaxApiProcess(t.shopApi.addCart, {
					gid: t.goodDetail.id,
					topicId: zbvd.topicid,
					sid: shopReq.sid,
					bid: shopReq.bid,
					...
					$("#yx_shop_intro #shop_intro_close").click(), $("#shopCar").html(e.total), $("#yx_shop_detail").hide(), $("ul.live-qiye-nav li.on").data("showcar") && $("#shopCar").show(), $(document).minTipsBox({
			...
			}), $(document).on(ClickOrTap, "#shopCar", function(e) {
				if (!zbvd.userid) return $(document).minTipsBox({
					tipsContent: "请先登录!",
					tipsTime: 1
				});
				t.preventScroll(), t.processCarList(!0), e.preventDefault()
				...
				if ($("#shop_area").closest(".admin-tk-link-cell").is(":hidden")) t.placeOrder(null, i, o);
					...
					"" != r && (d.aid = r), t.videoOpt(1), $("#loadingToast").show(), t.ajaxApiProcess(t.shopApi.newOrEditAddress, d, function(e) {
						$("#shop_address_aid").val(e.aid), t.placeOrder(e.aid)
					})
				}
			}), $(document).on("click", "#shop_payfeiBox #shop_pay_cancle", function(e) {
				$(document).popBox({
					boxContent: "确定要取消支付?",
					btnType: "both",
					confirmFunction: function() {
						$("#shop_payfeiBox,#shop_user_address,#div_carList").hide(), $("#shopCar").show()
					}
				})
			}), $(document).on("click", "#shop_payfeiBox #shop_pay_ok", function(e) {
				t.payProcess()
				...
				$("#shop_car_list .ul_car_shop li a.active").each(function(e, t) {
					if ("0" == $(t).data("isvirtual")) return o = !1, !1;
					var n = $(t).data("id"),
						s = $(t).data("gsps"),
						a = $(t).data("count");
					i.push({
				...
				}), o ? ($("#shop_area").closest(".admin-tk-link-cell").hide(), $("#shop_detail_address").closest(".admin-tk-link-cell").hide()) : ($("#shop_area").closest(".admin-tk-link-cell").show(), $("#shop_detail_address").closest(".admin-tk-link-cell").show(), $("#a_save_address").html($("#shop_address_aid").val() ? "确定" : "保存")), t.videoOpt(2), $("#shop_user_address").show(), $("#div_carList").hide()
			}), $(document).on(ClickOrTap, "a.a_car_sel", function(e) {
				$(this).toggleClass("active"), t.processCarList(), e.preventDefault()
			}), $(document).on(ClickOrTap, "li#not_jiesuan", function(e) {
				$(this).children("a").addClass("active").end().siblings().children("a").removeClass("active"), t.processCarList(), e.preventDefault()
			}), $(document).on(ClickOrTap, "li#ok_jiesuan", function(e) {
				$(this).children("a").addClass("active").end().siblings().children("a").removeClass("active"), $("#shop_car_list,#div_shopcar_bottom").hide(), $("#shop_jiesuan_list,#div_shopjisuan_bottom").show(), $("#loadingToast").show(), t.ajaxApiProcess(t.shopApi.getOrders, {
					topicId: zbvd.topicid,
					sid: shopReq.sid,
					bid: shopReq.bid
				}, function(e) {
					$("#loadingToast").hide(), laytpl($("#shopjisuanTpl").html()).render(e.datas || [], function(e) {
						$("div#shop_jiesuan_list div").remove(), $("div#shop_jiesuan_list").prepend(e)
					})
				}), e.preventDefault()
			}), $(document).on("click", "#div_look_orders", function(e) {
				window.location.href = shopReq.apiAddress + "buyerCenter?bid=" + shopReq.bid + "&sid=" + shopReq.sid
			}), $(document).on("click", ".i_car_num_plus", function(e) {
				t.carOpt.call(this, 1)
			}), $(document).on("click", ".i_car_num_reduce", function(e) {
		...
		carOpt: function(e) {
			...
			$("#loadingToast").show(), t.ajaxApiProcess(t.shopApi.updateGoodsOfCarts, {
				topicId: zbvd.topicid,
				sid: shopReq.sid,
				bid: shopReq.bid,
				gid: s,
				...
				$("#loadingToast").hide(), $(".look-buy-price", o).html(a * n), $(".a_car_sel", o).data("count", n), i.siblings(".i_car_num").html(n), t.processCarList()
			})
		},
		...
		initArea: function() {
			...
			zbvd.isadmin || $.post("/liveajax/GetShopAreaList", function(i) {
				if (i && i.length > 0) {
					var o = i;
					t.larea = new LArea, t.larea.init({
						trigger: "#shop_area",
						...
						if (i.data && i.data.length) {
							var n = i.data[0];
							$("#shop_consignee").val(n.truename), $("#shop_phone").val(n.mobile);
							var s = [];
							e(o, n.areaIds.split(","), s), t.larea.value = s, $("#shop_area_hidden").val(n.areaIds), $("#shop_area").val(n.areaName), $("#shop_detail_address").val(n.addr_info), $("#shop_address_aid").val(n.id), $("#a_save_address").html("确定")
		...
		loadData: function() {
			function e(e) {
				...
				laytpl($("#listTpl").html()).render(e, function(e) {
					$("div#shopList").prepend(e), tabSwiper && tabSwiper.onResize()
				})
			}
				...
				if (!(i.length <= 0)) {
					...
					t.ajaxApiProcess(t.shopApi.getGoods, {
						...
						for (var i = 0; i < t.datas.length; i++) for (var o = t.datas[i], n = 0; n < a.length; n++) if (a[n].yxId == o.id && 0 == o.status) {
							a[n].CurrentPrice = o.price, a[n].ProductName = o.name, a[n].PicIds = o.img, a[n].IsSellOut = o.inventory <= 0;
							break
						}
						e(a)
					}, function() {
		...
		loadvzyxData: function(e) {
			if (!t.loadFlag) {
				...
				$("#loadingToast").show(), $.post("/liveajax/GetvzyxList", {
					topicId: zbvd.topicid,
					searchStr: o,
					pi: i
				}, function(n) {
					...
					!n || n.length <= 0 ? (t.loadFlag = !0, !e && $(document).minTipsBox({
						...
						tipsTime: 1
					}), o ? $("#noshoptip").hide() : $("#noshoptip").show()) : (t.loadFlag = !1, i++, laytpl($("#vzyxlistTpl").html()).render(n, function(e) {
		...
		setShopTop: function(e) {
			t.log(e), $(".dianshang-head img").attr("src", -1 != e.PicIds.indexOf("</a>") ? $(e.PicIds).attr("href") : e.PicIds), $(".dianshang-hudong-title").html(e.ProductName), $(".dianshang-price").html(e.CurrentPrice), 0 == e.Type ? ($("#topShop").data("url", -1 != e.BuyLinkUrl.indexOf("</a>") ? $(e.BuyLinkUrl).attr("href") : e.BuyLinkUrl), $(".icon-tui-cart").hide()) : ($("#topShop").data("url", ""), $(".icon-tui-cart").data("id", e.yxId).show()), 0 != e.Type && e.IsSellOut || ($(".line-vote").hide(), $("#topShop").show()), $("body").addClass("liveshop")
		},
		goodDetail: null,
		ids: "",
		...
		processCarList: function(e) {
			function i() {
				...
				$("#shop_car_list .ul_car_shop li a.active").each(function(i, o) {
					...
					e.totalPiece += 1, e.totalPrice = t.floatAdd(parseFloat(n), e.totalPrice)
				}), $("#div_carList,#shop_car_list,#div_shopcar_bottom").show(), $("#shop_jiesuan_list,#div_shopjisuan_bottom").hide(), $("#div_shopcar_bottom #span_cal_num").html(e.totalPiece + "种商品"), $("#div_shopcar_bottom #span_cal_price").html(e.totalPrice.toFixed(2) + "元")
			}
			e ? ($("#loadingToast").show(), t.getCarList(function(e) {
				laytpl($("#shopCarlistTpl").html()).render(e, function(e) {
					$("div#shop_car_list ul").remove(), $("div#shop_car_list").prepend(e), $("#loadingToast").hide()
				}), i()
			})) : i()
		},
		placeOrder: function(e, i, o) {
			...
			e && (s.aid = e), i && (s.name = i), o && (s.mobile = o), t.ajaxApiProcess(t.shopApi.checkAndNewOrder, s, function(e) {
				$("#shop_payfeiBox #aText").html("总额:￥" + e.data.total_price + (0 != e.data.ship_price ? "(已含运费" + e.data.ship_price + "元)" : "")), t.oId = e.data.oid, $("#shop_payfeiBox").show(), t.isGoBuy = !1, t.goBuyData = null
			})
		},
		payProcess: function() {
			function e(e, t) {
			...
			function i(e, t) {
				WeixinJSBridge.invoke("getBrandWCPayRequest", JSON.parse(e), function(e) {
			...
			$.ajax({
				...
				error: function() {
					$("#loadingToast").hide(), redbagbol = !0, $(document).minTipsBox({
						tipsContent: "error",
						tipsTime: 1
					})
				}
		...
		floatAdd: function(e, t) {
			...
			return n = Math.pow(10, Math.max(i, o)), (e * n + t * n) / n
		},
		videoOpt: function(e) {},
		iswx: function() {
			return -1 != navigator.userAgent.toLowerCase().indexOf("micromessenger")
		...
		nodeScroll: function(e, t) {
			e.scroll(function() {
				nodeScrollTotalHeigth = e[0].scrollHeight, nodeScrollTop = e[0].scrollTop, nodeHight = e.height(), nodeScrollTotalHeigth - nodeScrollTop - nodeHight < 10 && t()
			})
		},
		ajaxApiProcess: function(e, i, o, n) {
			$.post(e, i, function(s) {
				return $("#loadingToast").hide(), t.log([e, i, s]), s ? 200 != s.code ? n ? n(s) : $(document).minTipsBox({
					tipsContent: s.msg,
					tipsTime: 1
	...
	return t
}), define("livecommon/module/fps", [], function() {
	var e = [],
		t = 0;
	return {
		init: function(e) {
		...
		getLatestFrames: function(t) {
			var i, o = [];
			for (t = t || 1, i = e.length - 1; i >= 0 && t > 0; i--) o.push(e[i]), t--;
			return o
		},
		getFPS: function() {
		...
		run: function() {
			var i = this;
			t || (t = (new Date).getTime()), window.requestAnimationFrame(function() {
				var o = (new Date).getTime();
				e.push(1e3 / (o - t)), e.length > 500 && e.shift(), t = o, i.run()
			})
		}
	}
}), define("livecommon/module/heart", ["livecommon/module/fps"], function(e) {
	var t = {
		...
		likeAnimationItem_qzone: function(e) {
			...
			return e = e || {}, t.push('<div class="heart'), i(e.special ? "-large" : ""), t.push(" "), i(e.batchId || ""), t.push('" id="'), i(e.id || ""), t.push("\">\n    <div class='heart-inner' style='background-image:url("), i(e.background || ""), t.push(")'></div>\n</div>"), t.join("")
		}
	},
		...
		n = {
			Init: function() {
				e.run(), this.BindEvent()
			},
			BindEvent: function() {
				$(".zan-click").on("click", function(e) {
					e.stopPropagation(), ++i, null != o && clearTimeout(o), o = setTimeout(function() {
			...
			fmtNumber: function(e) {
				e.toString().length > 4 && (e = e.toString().replace(/^\d+$/g, function(e) {
			...
			playLikeAnimation: function(e, i, o) {
				if (this.curLikeCount > 50) return logger.log("too many likes"), void(this.delayLike = {
					cnt: e,
					uid: i,
					special: o
				});
				var n, s, a = this,
					...
					r = [];
				for (e = e || 0, e = Math.min(e, 50), this.delayLike = null, s = 0; s < e; s++)(s + 1) % 10 == 0 && (o = !0), n = Math.ceil(12 * Math.random()), o ? Math.floor(10 * Math.random()) + 1 : 0, r.push({
							...
							return function() {
								...
								TweenMax.to(t, 3, {
									...
									onComplete: function(t) {
										return function() {
											e = e ? e.add($(t)) : $(t), e.size() > 20 && (e.remove(), i.curLikeCount = Math.max(0, i.curLikeCount - e.size()), e = null), i.delayLike && i.playLikeAnimation(i.delayLike.cnt, i.delayLike.uid, i.delayLike.special)
										}
									}(t),
									ease: Power1.linear
	...
	return n
}), define("livecommon/module/Praise", [], function(e, t) {
	var i = (new Date).getTime(),
		o = {
			...
			BindEvent: function() {
				o.goJudge(o.clickLovePC, o.clickLoveMO), $(".zan-click").on("click", function(e) {
					(new Date).getTime() > i + 200 && (popqueue.push(1, 1, 1), $.post("/liveajax/userpraise", {
						tpid: zbvd.topicid,
						v: (new Date).getTime()
					}, function(e) {
			...
			clickLovePC: function() {
				$(".zan-click").on("mousedown", function(e) {
					$(".toolmenu .zan-box i").css("color", "rgb(245, 28, 48)")
				}), $(".zan-click").on("mouseup", function(e) {
					$(".toolmenu .zan-box i").css("color", "#ff3d03")
				})
			},
			clickLoveMO: function() {
				$(".zan-click").on("touchstart", function(e) {
					$(".toolmenu .zan-box i").css("color", "rgb(245, 28, 48)")
				}), $(".zan-click").on("touchend", function(e) {
					$(".toolmenu .zan-box i").css("color", "#ff3d03")
				})
			},
			SetTopicPraise: function(e, t) {
				...
				if (e > 0 && (i = e, $(".zan-num").data("praise", e)), i.toString().length > 4) {
					...
					null != o && (i = o.toString().replace(/^\d+$/g, function(e) {
						return (e / 1e4).toFixed(2) + "万"
					}))
				}
				$(".zan-num").html(i)
			...
			sendLove: function(e, t, i, o, n) {
				...
				$(e).append("<img class = 'loveimg' src=''>"), $(e).children().eq(a).attr("src", "https://j.vzan.cc/zhibo/livecontent/livecommon/images/flash/kawayi" + s + ".png"), d < t / 3 && $(".loveimg:last").animate({
					bottom: o,
					opacity: "0",
					left: -d,
					width: 40,
	...
	return o
}), define("livecommon/module/controls", ["livecommon/module/utils"], function(e) {
	var t = function(e, t) {
		...
		i = {
				...
				if (void 0 == t) {
					$.post("/live/GetMiniQRSrc", {
						...
						if (t && t.isok) if ("" != t.dataObj.baseimg) {
							...
							e.drawImageByCanvas(344, 344, o, function(e) {
								$(".showAuctionQrcode").data("xcqQRImg", e), i.popupCode("进入小程序参与竞拍，体验0延迟", e)
							})
						} else $(".showAuctionQrcode").data("xcqQRImg", t.Msg), i.popupCode("进入小程序参与竞拍，体验0延迟", t.Msg)
					}, "json")
				} else i.popupCode("进入小程序参与竞拍，体验0延迟", t)
			},
			popupCode: function(e, i) {
				$("#alertCodeMain").remove(), $("body").append(t(e, i)), $(".close-button,.tk-mask").on("click", function() {
			...
			popupMiniCode: function(t) {
				...
				e.IsPC() && (i = '<p style="text-align: center;padding: 12px 0px 6px 0px;color: #353535;font-size: 14px;">扫描小程序二维码看直播</p><img src=\'' + t + "' width='250px' height='250px' style='margin:0px 20px;' /><p style=\"text-align: center;padding: 0px 0px 16px 0px;color: #353535;font-size: 14px;\"><img src=\"//j.vzan.cc/zhibo/livecontent/img/miniQRksIcon.png\" style=\"width:16px;height:20px;vertical-align:middle;\" /> 请使用微信扫描二维码</p>"), layer.open({
					type: 1,
					anim: 5,
					title: !1,
					shadeClose: !0,
	...
	return i
}), window.videoW = 0, window.videoH = 0, window.fswinH = screen.height, window.fswinW = screen.width, window.normalwinH = window.innerHeight, window.calvideoH = 0, window.vduration = 0, window.nextSeekTime = -1, define("livecommon/module/wxvideoplayer", ["livecommon/module/utils"], function(e) {
	function t(e) {
		if (!e || e == 1 / 0) return "00:00";
		var t = parseInt(e / 60),
			i = parseInt(e % 60),
			o = 0;
		return t >= 60 && (o = parseInt(e / 3600), t -= 60 * o), t = t >= 10 ? t : "0" + t, i = i >= 10 ? i : "0" + i, o = o >= 10 ? o : "0" + o, o > 0 ? o + ":" + t + ":" + i : t + ":" + i
	}
	function i() {
	...
	function n() {
		...
		} else viewtimes = 0, clearInterval(v)
	}
	function s() {
		var e = $(".progress"),
			n = e.find(".p-dot"),
		...
		if (!p) {
			var l = o() / i(),
				u = a * l;
			n.css("left", u - 15 + "px"), s.css("width", u + "px")
		}
		$(".time-current").html(d), $(".time-duration").html(c), r.LimitFreeView()
	}
	function a(e) {
		e = e || 6, null != b && (clearTimeout(b), b = null), b = setTimeout(function() {
			r.HideIconOrControls(), w = !1
		}, 1e3 * e)
	}
	var d, r = this,
		c = e.IsPC(),
		...
		k = null,
		_ = 0;
	r.IsWeChat = e.IsWeiXinChat(), r.hostname = window.location.host, r.UseNewX5Player = !e.isMobileQQBrowser() || e.isWapBrowser() && ("Vertical" != zbvd.tplname || 0 == zbvd.IsScreen), r.isloading = !1, r.isgetstatusloading = !1, r.outsetAd = null, r.pauseAd = null, r.liveinfo = null, r.adUrl = "", r.isPlayAd = !1, r.playAdType = "", r.AdCountDownTimer = null, r.isPlayAded = !1, r.LimitFreeViewTimes = 0, r.viewtimes = 0, r.isBindEvent = !1, r.IsChangeUrl = !1, r.HideIconOrControls = function() {
		...
		w = !0, $(".icon-back-index,.icon-refresh,.refresh-page").show(), "1" == $(".btnShowMiniAppIcon").data("isopen") && $(".btnShowMiniAppIcon").show(), $("#videoPlayer video").length > 0 && (0 == $("#div_live_status").length || $("#div_live_status").is(":hidden") || r.isPlayAd) && $(".controls").show(), r.LimitFreeViewTimes > 0 && r.LimitFreeViewTimes && $(".controls").show()
	}, r.CheckVideoIsExitfullscreen = function() {
		return console.log("CheckVideoIsExitfullscreen"), !1
		...
		r.isgetstatusloading = !0, $.post("/liveajax/GetTopicStatus", {
			...
			if (e && e.isok) {
				...
				if (t) if (1 == t.status)"notsignal" == r.liveinfo.livestatus && t.playurl && (l.find("p").text("正在加载实时直播"), 1 != parseInt(r.liveinfo.isScreen) && setTimeout(function() {
					layer.msg("正在加载实时直播")
				}, 12e3)), "beginning" != r.liveinfo.livestatus && "" != t.playurl && setTimeout(function() {
					r.liveinfo.livestatus = "beginning", r.liveinfo.playurl = t.playurl, r.startPlay(!1, !0)
				}, 12e3);
				else if ((0 == t.status || -1 == t.status) && "beginning" == r.liveinfo.livestatus) {
					var i = 7e3;
					r.liveinfo.playbackurl = t.playurl, t.playurl ? (i = 12e3, $(".new-count-down.vertical-countdown").css("top", "31%"), l.find("p").text("直播暂停中，观看回放"), 1 != parseInt(r.liveinfo.isScreen) && setTimeout(function() {
						layer.msg("直播暂停中，正在加载实时回放")
					}, i)) : ($(".vertical-view .fsl-bg").css("background", "url(" + r.liveinfo.cover + ") center no-repeat #000"), $(".video-poster").attr("src", r.liveinfo.cover), l.find("p").text("正在加载")), setTimeout(function() {
						r.liveinfo.livestatus = "notsignal", r.startPlay()
					}, i)
				}
			}
		...
		if (null != r.AdCountDownTimer && (clearTimeout(r.AdCountDownTimer), r.AdCountDownTimer = null), $(".plan-name").empty().html($(".plan-name").data("qrname")), r.isPlayAd && "outsetad" == r.playAdType && r.SetAdViewedCookie(), r.isPlayAd = !1, window.videoW = 0, $(".vertical-new-btns").hide(), $(".vertical-new-countdown").hide(), $("#videoPlayer").css("height", "auto"), $("#liveVideo").css("height", $(".video-poster").height() + "px"), "notbegin" == r.liveinfo.livestatus || "notsignal" == r.liveinfo.livestatus || "endlive" == r.liveinfo.livestatus) {
			...
			var o = "",
				n = "";
			"notsignal" == r.liveinfo.livestatus && ($("#div_live_status").addClass("new-count-down"), !t && r.pauseAd.isOpen && "" != r.pauseAd.videolink && (r.isPlayAd = !0, o = r.pauseAd.videolink)), !o && r.liveinfo.playbackurl ? (n = "直播暂停中，观看回放", o = r.liveinfo.playbackurl) : t && "" == o ? (m.pause(), $("#videoPlayer").empty(), $("#videoPlayer").css({
				width: "100%",
				height: "auto"
			...
			r.HideLoadingBox()
		}, 1500), $("#videoPlayer video").attr("src", r.liveinfo.playurl), $(".controls .progress .p-play").css("width", "0px"), $(".controls .progress .p-dot").css("left", "-1.5rem"), r.IsChangeUrl = !0, m.currentTime = 0, l.show(), setTimeout(function() {
			m.play()
		}, 1500);
			...
			if ($("#videoPlayer video").length > 0) {
				...
				if (d == p) return !1;
				window.videoW = 0, $("#videoPlayer video").attr("src", r.liveinfo.playurl), e.IsIOS() || r.UseNewX5Player ? (m.play(), s = 100) : (s = 100, $("body").addClass("wxfs")), a = !$("#videoPlayer video").is(":hidden"), a && $("#livePoster").hide(), r.IsChangeUrl = !0
			} else {
				...
				$(".videoPoster").length <= 0 && (g += "<div class='videoPoster'>", g += "<img id='playbtn' style='top: 50%;left: 50%;-webkit-transform: translate(-50%,-50%);transform: translate(-50%,-50%);position: absolute;cursor:pointer;display:none;' src='//j.vzan.cc/zhibo/img/play.png' width='80px'>", g += "<img src='" + r.liveinfo.cover + "' style='cursor:pointer;border:0;width:100%;' id='livePoster' />", g += "</div>"), $("#videoPlayer").prepend(g), $(".video-poster").hide(), $(".vertical-new-countdown").hide(), setTimeout(function() {
					m = null, m = document.getElementById("liveVideo"), f = $(m), x.BindVideoEvent(), x.BindEvent()
				}, 550)
			}
			setTimeout(function() {
				a || $("#livePoster").show(), r.liveinfo.playbackurl || t ? $(".controls,.btn-pause").show() : ($(".videoPoster,#playbtn,.btn-play").show(), $(".controls,.btn-pause").hide()), $(".controls .timestatus").html("实时直播"), $(".controls .timestatus").show(), $(".controls .progress,.time").hide(), $("#div_live_status").hide(), m.play()
		...
		if (r.isPlayAd) {
			var e = '<span id="t" style="color:red;">' + parseInt(m.duration) + "</span> 秒";
			(r.outsetAd.isskip || r.pauseAd.isskip) && (e += '<span class="skipad">跳过</span>'), $(".plan-name").html(e), $(".plan-name,.progress").show(), $(".controls .timestatus").html(""), $(".controls .timestatus").hide(), r.AdCountDown()
		}
		...
		if (r.LimitFreeViewTimes > 0 && !r.isPlayAd) {
			u = !1, r.viewtimes++, $("#div_live_status").show();
			...
			var t = 0,
				...
				o = 0,
				n = 0;
			e > 0 && (t = Math.floor(e / 86400), i = Math.floor(e / 3600) - 24 * t, o = Math.floor(e / 60) - 24 * t * 60 - 60 * i, n = Math.floor(e) - 24 * t * 60 * 60 - 60 * i * 60 - 60 * o), t <= 9 && (t = "0" + t), i <= 9 && (i = "0" + i), o <= 9 && (o = "0" + o), n <= 9 && (n = "0" + n), $("#showday var").html(t), $("#showhour var").html(i), $("#showminute var").html(o), $("#showsecond var").html(n)
		}
	}, r.GetLiveInfo = function() {
		return r.outsetAd = JSON.parse(sessionStorage.getItem("outsetAd_" + zbvd.topicid) || "{}"), r.pauseAd = JSON.parse(sessionStorage.getItem("pauseAd_" + zbvd.topicid) || "{}"), r.liveinfo = JSON.parse(sessionStorage.getItem("liveinfo_" + zbvd.topicid) || "{}"), r.liveinfo.tpid ? !(r.liveinfo.playurl.indexOf(".mp4") <= -1 && r.IsWeChat && c) && void("notsignal" != r.liveinfo.livestatus && "notbegin" != r.liveinfo.livestatus || setTimeout(function() {
			r.GetLiveStatus()
		...
		if (e) m = e, f = $(m), setTimeout(function() {
			x.BindEvent(), x.BindVideoEvent()
		}, 600);
		else if (r.liveinfo) {
			if (r.liveinfo.playurl.indexOf(".mp4") <= -1 && r.IsWeChat && c) return !1;
	...
	var x = {
		Init: function(e) {
			...
			r.InitPlayer(), r.LimitFreeViewTimes = $(".countdown-tip").data("time") || 0
		},
		BindVideoEvent: function() {
			if (r.IsWeChat && c) return !1;
			e.iscommonleveltbs() && (r.UseNewX5Player || m.addEventListener("x5videoenterfullscreen", function() {
				setTimeout(function() {
					console.log("inall"), 0 == fswinH && (fswinH = screen.height), 0 == fswinW && (fswinW = screen.width()), $("#videoPlayer").css({
						width: "100%",
						height: 257
					}), $("#liveVideo").css({
						width: "100%",
				...
				}, 800), $("body").addClass("wxfs"), "beginning" != r.liveinfo.livestatus || r.isPlayAd || $(".btn-play").hide(), $(".btn-pause").show(), window.navigator.userAgent.indexOf("MI 8") > -1 && ($(".inputbox").addClass("mi8input"), $("#inputbar").addClass("mi8input-container"), $(".inputbox input").on("blur", function() {
					x.ReMakePage()
				}))
			}), m.addEventListener("x5videoexitfullscreen", function() {
				if (console.log("exitall"), f.removeAttr("x5-video-orientation"), $("#videoPlayer").css("height", "auto"), $("body").removeClass("tbsfs wxfs"), 0 != parseInt(r.liveinfo.isScreen)) {
					var e = fswinW,
						t = e / videoW * videoH;
					window.calvideoH != t && (window.calvideoH = t), window.calvideoH = e / videoW * videoH, $("#videoPlayer").css({
						width: "100%",
						height: window.calvideoH
					}), window.calvideoH > 0 ? $("#videoPlayer video").height(window.calvideoH) : ($("#videoPlayer video").hide(), $("#videoPlayer #livePoster").show()), x.ReMakePage("normal")
				}
				setTimeout(function() {
					$(".btn-play").show()
				}, 1e3), r.ShowIconOrControls(), $(".btn-pause").hide(), $(".return-live-btn").show(), $("body").css("overflow-y", "auto"), 1 === zbvd.IsScreen && setTimeout(function() {
					x.resizeContentHeight()
				}, 500), $("#inputbar").css("bottom", 0), $(".inputbox input").removeAttr("disabled"), $(".inputbox").removeClass("mi8input"), $("#inputbar").removeClass("mi8input-container")
				...
				m && 2 == zbvd.modeltype && !e.iscommonleveltbs() && "Vertical" != zbvd.tplname && parseInt(r.liveinfo.isScreen) && $("#liveVideo").length > 0 && setTimeout(function() {
					window.videoW = m.videoWidth, window.videoH = m.videoHeight;
				...
				setTimeout(function() {
					if (x.resizeContentHeight(), window.videoW > 0 && window.videoH > 0 && "none" != $("#liveVideo").css("display")) {
						var e = $("#LiveVideoArea").width() - (c ? 14 : 0);
						window.calvideoH = e / videoW * videoH, $("body").hasClass("wxfs") && (window.calvideoH += 50), $("#videoPlayer").css({
							width: "100%",
							height: window.calvideoH
						}), console.log(" window.calvideoH:--", window.calvideoH, $("#LiveVideoArea").width())
					}
				}, 500)
			}, !1), f.on("end playing", function() {
				var e;
				setTimeout(function() {
					l.hide(), e = setInterval(function() {
						l.hide(), l.is(":hidden") && (clearInterval(e), console.log("clearInterval"))
					}, 800)
				}, 800)
			}).on("loadstart", function() {
				console.log("loadstart"), $("#liveVideo").length > 0 && "" != $("#liveVideo").attr("src") && l.show()
			}), m.addEventListener("error", function(e) {
				console.log("error")
			}), m.addEventListener("loadstart", function(e) {
				console.log("loadstart"), l.show()
			}), m.addEventListener("waiting", function(e) {
				console.log("waiting"), l.show(), setTimeout(function() {
					l.hide()
				}, 1e3)
			}), m.addEventListener("playing", function() {
				console.log("playing"), _ = 0, clearInterval(d), $("#div_live_status").hide(), r.hostname.indexOf("vzan.com") <= -1 && ("" != zbvd.rightTopWatermark || r.isPlayAd) && !e.IsIOS() && 1 == zbvd.IsScreen && $(".plan-name").css("padding-right", "40px"), r.SetAdCountDownStr(), l.find("p").text("正在加载"), $(".btn-pause").show(), $(".videoPoster").hide(), $("#playbtn, .btn-play").hide(), setTimeout(function() {
				...
				}, 1e3), r.IsChangeUrl && (window.videoW = 0, $("#videoPlayer").css({
					width: "100%",
					height: "auto"
					...
					width: "100%",
					height: "auto"
				}), m = null, m = document.getElementById("liveVideo"), r.IsChangeUrl = !1), setTimeout(function() {
					!(window.videoW <= 0 || window.videoH <= 0) || 0 == parseInt(r.liveinfo.isScreen) && "smallvideo" != window.liveStatus ? x.ReMakePage() : (null != y && (clearInterval(y), y = null), y = setInterval(function() {
						if (m.videoWidth > 0 && m.videoHeight > 0 && $("#liveVideo").length > 0) {
							clearInterval(y), y = null, window.videoW = m.videoWidth, window.videoH = m.videoHeight;
							...
							window.calvideoH != t && (window.calvideoH = t), window.calvideoH = e / videoW * videoH, $("#videoPlayer").css({
								...
								width: "100%",
								height: window.calvideoH
							}), console.log("window.calvideoH:" + window.calvideoH + "showWidth:" + e), x.ReMakePage()
						}
					}, 800))
				}, 1e3), "playback" == r.liveinfo.livestatus || "videosrc" == r.liveinfo.livestatus || "smallvideo" == r.liveinfo.livestatus || r.isPlayAd || "notsignal" == r.liveinfo.livestatus && r.liveinfo.playbackurl ? ($(".progress,.time").show(), $(".controls .timestatus").hide(), null != k && (clearInterval(k), k = null), k = setInterval(function() {
					console.log(m.duration), m.duration != 1 / 0 && m.duration > 0 && (clearInterval(k), k = null, window.vduration = m.duration, -1 != window.nextSeekTime && m && (m.currentTime = window.nextSeekTime, window.nextSeekTime = -1), x.ShowProcess(), a(), !r.isPlayAd && timingredbag && timingredbag.StartViewTimeClock())
				...
				console.log("Event:pause"), $("#videoPlayer video").attr("src") && $(".btn-play,#playbtn").show(), $(".btn-pause").hide(), l.hide(), r.ShowIconOrControls(), $(".videoPoster").show(), f.is(":visible") ? ($(".controls").show(), $("#livePoster").hide()) : ($("#livePoster").show(), $("#videoPlayer").css("height", "auto")), $("#liveVideo").length < 0 && $("#playbtn").hide(), timingredbag && timingredbag.StopViewTimeClock()
			}, !1), m.addEventListener("loadstart", function() {
				console.log("loadstart")
			}), m.addEventListener("loadeddata", function() {
				console.log("loadeddata"), _ = 0, "notsignal" == r.liveinfo.livestatus && r.liveinfo.playbackurl && (d = setInterval(function() {
		...
		BindEvent: function() {
			...
			}).on("click", ".skipad", function(e) {
				r.isPlayAd && (r.isPlayAded = !0, m && m.pause(), r.startPlay(!0)), e.stopPropagation()
			}), $(document).on(c ? "mousedown" : "touchstart", ".progress .p-dot", function(e) {
				function i(e) {
					x.Pause(), a(), e.stopPropagation(), e = c ? e.originalEvent.pageX : e.originalEvent.touches[0].pageX, e = h + (e - u), 0 > e && (e = 0), e > v && (e = v), g = e / (v - 0);
					var i = (l * g).toFixed(0);
					s.css("left", String(f * g - 9) + "px"), d.css("width", String(f * g) + "px"), $(".time-current").text(t(i))
				}
				function o(e) {
					return a(), e.stopPropagation(), e = Number((V * g).toFixed(3)), m.currentTime = e, x.Play(), p = !1, $(document).off(c ? "mousemove" : "touchmove"), $(document).off(c ? "mouseup" : "touchend"), !1
				}
				if (!(r.LimitFreeViewTimes > 0) || r.isPlayAd) {
					p = !0;
					var n = $(".progress"),
					...
					return $(document).on(c ? "mousemove" : "touchmove", i), $(document).on(c ? "mouseup" : "touchend", o), e.stopPropagation(), e.preventDefault(), !1
				}
			}), $(document).on("click", ".btn-play", function(e) {
				x.Play(), a(), e.stopPropagation()
			}), $(document).on("click", ".btn-pause", function(e) {
				x.Pause(), null != b && clearTimeout(b), e.stopPropagation()
			}), $("#wonderful_default_tv").click(function() {
				$("#liveVideo").attr("src", $("#live_source_url").val()).attr("woderful_tv_id", 0), $("#playbtn").click(), x.changeWondfulShareUrl(0)
			})
		},
		...
		ReMakePage: function(t) {
			if ("Vertical" != zbvd.tplname && parseInt(r.liveinfo.isScreen)) {
				...
				if ("normal" == t || e.IsIOS() || !e.iscommonleveltbs()) i = normalwinH - window.calvideoH - ($(".live-qiye-nav").height() || 0) - ($(".live-qiye-notice").height() || 0) - ($(".undervideoimg").height() || 0);
					...
					$("body").hasClass("wxfs") ? o += 50 : $("#inputbar").css("bottom", 0), $("#videoPlayer").css({
						width: "100%",
						height: o
				...
				window.navigator.userAgent.indexOf("MI 8") > -1 && $("body").hasClass("wxfs") && (i -= 88), $(".scrollContentBox").css({
					height: i
		...
		resizeContentHeight: function() {
			...
			a = a <= 80 ? 280 : a, $(".scrollContentBox, #tab-content-msgwrapper, #tab-content-msgwrapper > .swiper-slide").height(a)
		},
		StartClock: function() {
			null != g && (clearInterval(g), g = null), console.log("StartClock"), g = setInterval(function() {
				s()
			}, 1e3)
		},
		StopClock: function() {
		...
		StartViewTimeClock: function() {
			null != v && (clearInterval(v), v = null), console.log("StartViewTimeClock"), v = setInterval(function() {
				n()
			}, 1e3)
		},
		StopViewTimeClock: function() {
			console.log("StopViewTimeClock"), null != v && (clearInterval(v), v = null)
		},
		Play: function() {
			m.play(), $(".vertical-new-btns").hide(), x.StartClock()
		},
		Pause: function() {
			m.pause(), x.StopClock(), $(".vertical-new-btns").show()
		},
		SetCurrentTime: function(e) {
			m.currentTime = e, m.play(), setTimeout(function() {
				l.hide()
			}, 1e3)
		},
		ShowProcess: function() {
			$(".time,.controls").show(), x.StartClock()
		},
		...
		changeWondfulShareUrl: function(e) {
			...
			t = -1 != n ? t.substring(0, n) + "wfid=" + e : t + "&wfid=" + e, $(".live_title_bar").attr("attr-url", t);
			...
			if (-1 != a) {
				var d = s[a];
				$(".live_title_bar").attr("attr-title", d.title).attr("attr-pic", d.cover).attr("attr-descript", d.remark), $("#liveVideo").attr("woderful_tv_id", d.Id).attr("src", d.playurl);
				var r = $(".wonderful_tv[data-id='" + d.Id + "']").find("img").attr("src");
				$("#livePoster").attr("src", r), console.log(d), document.title = d.title
			}
			console.log(t), x.wxShareShow()
		},
			...
			wx.onMenuShareAppMessage({
				...
				success: function() {
					$(this).minTipsBox({
						tipsContent: "分享成功",
						tipsTime: 1
					}), "AfterShareCouponCode" == s && AfterShareCouponCode(), "function" == typeof menuShareCallBack && menuShareCallBack()
				},
				cancel: function() {}
	...
	return window.resizeContentHeight = x.resizeContentHeight, x
}), define("livecommon/module/lottery", ["livecommon/module/utils", "jquery.popBox"], function(e, t) {
	return {
		Init: function() {
			this.BindEvent()
		},
		...
		handleLottery: function(e, t) {
			var i = this;
			if (0 == e) {
				if (!t || t.length <= 0) return $(".lottery_cj_back").show(), popup.confirm("温馨提示", "抱歉,当前没有在线用户或在线用户已经成为幸运观众", ["确定"], function() {
					this.hide()
				}), !1;
				$("#live_lottery_boxRight,#live_lottery_boxLeft").addClass("none"), $("#live_lottery_boxFront,#live_lottery_boxImg").removeClass("none"), $("#live_lottery_shou_show").removeClass("none"), setTimeout(function() {
					$("#live_lottery_shou_show").addClass("none"), $("#live_lottery_getwin").removeClass("none"), i.switchWinImg(t, winUs.length), setTimeout(function() {
						$("#live_lottery_getwin,.lottery_gun2").addClass("none");
						for (var e = "", t = 0; t < winUs.length; t++) e += '<li>                                            <a class="huojiang-pic"><img src="' + winUs[t].headimgurl + '"></a>                                            <p class="name">' + winUs[t].nickname + "</p>                                            </li>";
						$("#live_lottery_winlist").find("li").remove().end().append(e), $(".xingyunguanzhong,#live_lottery_winlist,.lotter_bg2").removeClass("none"), $(".juanzhouzhi").addClass("juanzhou-active"), $(".lottery_cj_back").show()
					}, 1e3 * t.length)
				}, 1e3)
			} else $("#live_lottery_awardstext").html($("#live_lottery_set_awards option[value=" + msg.Msg.tuserId + "]").text()), $(".lottery_gun2").removeClass("none"), $(".xingyunguanzhong,#live_lottery_winlist,.lotter_bg2").addClass("none"), $(".juanzhouzhi").removeClass("juanzhou-active"), $("#live_lottery_boxImg,#live_lottery_boxFront,#live_lottery_getwin").addClass("none"), $("#live_lottery_boxLeft,#live_lottery_boxRight").removeClass("none"), $("#live_lottery_box").show(), zbvd.isadmin || $(".lottery_cj_back").hide()
		}
	}
});
var wss = null,
	...
	weddingIndex = 0,
	dropTimer = !1;
define("livecommon/module/im", ["livecommon/module/utils", "livecommon/module/redbag", "livecommon/module/shop", "livecommon/module/heart", "livecommon/module/Praise", "livecommon/module/controls", "livecommon/module/wxvideoplayer", "livecommon/module/lottery"], function(utils, RedBag, shop, heart, Praise, liveControl, wxvideoplayer, lottery) {
	juicer.set({
		"tag::operationOpen": "{#",
		"tag::operationClose": "}",
		"tag::interpolateOpen": "${",
	...
	juicer.register("timefmt_build", timefmt), juicer.register("recommended_build", recommendedbtn), juicer.register("hasTime_build", hasTime), juicer.register("showTime_build", showTime), juicer.register("showFile_build", showFile);
	var dm_top = -35,
		isScrollBottom = !0;
	$(function() {
		$("#speakBox").scroll(function(e) {
			$("#speakBox").scrollTop() + $("#speakBox").height() < $("#speakBox .liveBoxContent").innerHeight() - 250 ? isScrollBottom = !1 : (isScrollBottom = !0, msgUtils.ResetUnreadMsgCount()), e.stopPropagation()
		}), $("#speakBox .liveBoxContent").scroll(function(e) {
			$("#speakBox .liveBoxContent").scrollTop() + $("#speakBox .liveBoxContent").height() < $("#speakBox .liveBoxContent div").innerHeight() - 250 ? isScrollBottom = !1 : (isScrollBottom = !0, msgUtils.ResetUnreadMsgCount()), e.stopPropagation()
		})
	}), juicer.register("nicknameFilters", function(e) {
		return e.length <= 3 ? e : e.substr(0, 2) + "..." + e.substr(e.length - 1, 1)
	}), juicer.register("nicknameReplace", function(e) {
		var t = e.content,
			i = e.tnickName;
	...
	var msgUtils = {
		...
		ResetUnreadMsgCount: function() {
			var e = $(".unreadMsgTip");
			e.find("var").text(0), e.removeAttr("firstmsgid"), e.hide()
		},
		showNormal: function(e) {
			function t(e) {
				var t = $("#tpl_recpro_" + e.content);
				$("#tpl_recprobanner_" + e.content).remove(), t.parent("li").remove(), t.parent("li").next(".news-alert-time").remove(), t.parent("li").prev(".news-alert-time").remove(), imwebsocket.delLiveMsgHandle(t.parent("li").attr("attr-id")), queryUtils.loadRecoProduct()
			}
			if (1 == zbvd.levels && (e.UserInfo = e.UserInfo || {}, e.UserInfo.RoleId = 0, e.UserInfo.RoleName = ""), 39 == e.Msg.msgtype) e.goods = JSON.parse(e.Msg.content);
			else if (43 == e.Msg.msgtype || 44 == e.Msg.msgtype) {
				44 == e.Msg.msgtype && (e.Msg.msgtype = 1);
				var i = JSON.parse(e.tempid);
				i && (i.rolename && (e.UserInfo.RoleName = i.rolename), i.nickname && (e.UserInfo.nickname = i.nickname), i.userphoto && (e.UserInfo.headimgurl = i.userphoto), 1 != i.openShang && "1" != i.openShang || (e.UserInfo.RoleId = 4)), e.tempid = ""
			...
			if ($("#chat-msglist li").length > 40 && isScrollBottom && $("#chat-msglist li").slice(0, $("#chat-msglist li").length - 40).remove(), ("" != zbvd.skin || "2" == zbvd.shangType) && 6 == e.Msg.msgtype && "" != e.Msg.ParentId) {
				...
				try {
					...
					wishedImg = e.Msg.ParentId.split("|")[0], wishedPrice = '<font style="font-weight:bolder;max-width: 40%;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">' + e.UserInfo.nickname + '</font>给主播祝福了<font style="font-weight:bolder;padding-left:3px;">¥' + (.01 * parseInt(e.Msg.ParentId.split("|")[1])).toFixed(2) + " 元</font>", wishedTxt = e.Msg.ParentId.split("|")[2], n.find("img").attr("src", wishedImg).end().find(".price").html(wishedPrice).end().find(".wish-txt").text(wishedTxt), setTimeout(function() {
						n.fadeIn(), showWish = !1
					}, 300), setTimeout(function() {
						n.fadeOut(), showWish = !0
					}, 6e3)
				} catch (e) {}
			}
			if (49 === e.Msg.msgtype && "提问" == e.Types)"function" == typeof getSocketQuestion && getSocketQuestion(e);
			else if (50 === e.Msg.msgtype) imdebug && console.log("remindAllRight"), "function" == typeof getSocketQuestion && remindAllRight(e);
				...
				1 == s.votetype ? $(".vertical-vote-icon").children("a").attr("href", "/live/votetext?tpid=" + zbvd.topicid + "&voteid=" + s.id + "&datatype=" + s.datatype) : $(".vertical-vote-icon").children("a").attr("href", "/live/VoteSmallVodieAndPic?tpid=" + zbvd.topicid + "&voteid=" + s.id + "&datatype=" + s.datatype), $(".vertical-vote-icon").show(), $(".line-vote").html(juicer($("#tpl_topvote").html(), s)).fadeIn()
				...
				if ("-1" == e.Msg.ParentId) try {
					...
					"[object Array]" == Object.prototype.toString.call(a) ? $.each(a, function(e, i) {
						t(i)
					}) : t(e.Msg)
				} catch (i) {
					t(e.Msg)
					...
					var a = JSON.parse(e.Msg.content);
					a.link = -1 == a.freight ? a.Subtitle : "/live/product-" + a.Id + "?ztpid=" + zbvd.topicid, $(".line-vote").hide(), $(".line-Product").html(juicer($("#tpl_recprobanner").html(), {
						goods: a
					})).fadeIn()
				}
				...
				case 0:
				case 2:
					"none" == $(".showAuctionQrcode").css("display") && $(".showAuctionQrcode").show(), $("#showAuctionIcon").attr("src", $("#showAuctionIcon")[0].src.replace("nobegin", "begin")), liveControl.popupAuctionQrcode();
					break;
				case 1:
				case 3:
					...
					break;
				case -1:
					$(".showAuctionQrcode").hide()
				}
				return !1
			}
			if (isScrollBottom) scrollbottomtimer && (clearTimeout(scrollbottomtimer), scrollbottomtimer = null), scrollbottomtimer = setTimeout(function() {
				zbvd.IsScreen && "2" == zbvd.modeltype && zbvd.ouPaiCustomId.indexOf("," + zbvd.zbid + ",") > -1 ? utils.moveToEndByOuPai($(".liveBoxContent"), function() {}) : utils.moveToEnd($("#speakBox"), function() {})
			}, 1);
				...
				0 == c && r.attr("firstmsgid", e.Msg.Ids), r.find("var").text(c += 1), r.show()
			}
			if (1 == e.Msg.msgtype && "" != e.Msg.ParentId && 0 != zbvd.userid && e.Msg.tuserId == zbvd.userid) {
				var l = $(".huifu-alert");
				l.attr("lastmsgids", e.Msg.Ids), l.show()
			}
				...
				"Shop" != zbvd.tplname && 1 != zbvd.shopMenu || (1 == zbvd.IsScreen ? setTimeout(function() {
					$.post("/live/GetNewVote", {
						...
						if (e && e.isok && "" != e.Msg) {
							...
							t.link = -1 == t.freight ? t.Subtitle : "/live/product-" + t.Id + "?ztpid=" + zbvd.topicid, $("#tpl_recprobanner").length > 0 && ($(".line-vote").hide(), $(".line-Product").html(juicer($("#tpl_recprobanner").html(), {
								goods: t
							})).fadeIn())
						}
					})
					...
					$.post("/live/getrecomendinfo", {
						...
						if (e && e.isok && e.dataObj) {
							var t = e.dataObj;
							t.link = -1 == t.freight ? t.Subtitle : "/live/product-" + t.Id + "?ztpid=" + zbvd.topicid, $("#tpl_recprobanner").length > 0 && $(".line-Product").html(juicer($("#tpl_recprobanner").html(), {
								goods: t
							})).fadeIn()
						}
					})
		...
		imwebsocket = {
			Init: function() {
				this.BindEvent(), zbvd.block && imwebsocket.BlockUser(), zbvd.blockall && ((parseInt(zbvd.roleid) || 0) <= 0 || zbvd.repayId > 0) && imwebsocket.BlockAll(), imwebsocket.GetHistoryMsg(function() {
				...
				}), queryUtils.loadRecoProduct(), setTimeout(function() {
					$.post("/live/GetNewVote", {
						tpid: zbvd.topicid,
						zbid: zbvd.zbid,
						msgtype: 25
				...
				}, 500), heart.Init(), WebSocket && null == wss && imwebsocket.ConnectServer(), null == pptSwiper && (pptSwiper = new Swiper(".swiper-container.liveppt", {
			...
			BindEvent: function() {
				wxvideoplayer.resizeContentHeight(), tabSwiper = new Swiper("#tab-content", {
					useCSS3Transforms: !1,
					autoHeight: !0,
					lazyLoading: !0,
					preloadImages: !1,
					onSlideChangeStart: function(e) {
						22 == $(".live-qiye-nav li").eq(e.activeIndex).attr("menustate") ? $("#oupai-custom").show() : $("#oupai-custom").hide(), wxvideoplayer.resizeContentHeight()
					},
					...
					onTransitionEnd: function(e) {
						...
						if (22 == i) {
							...
							o.attr("style", n), $(".inputbar,.toolmenu").show(), $("#new-menu-btn").css("display", "none")
						} else $(".inputbar,#c-gift,.toolmenu").hide(), $("#new-menu-btn").css("display", "block"), 19 == i ? $(".interact-btn").show() : $(".interact-btn").hide();
						t.data("showcar") ? zbvd.isadmin ? $("div.look-buy-bottom-nav-fix").show() : $("#shopCar").show() : ($("#shopCar").hide(), $("div.look-buy-bottom-nav-fix").hide()), $(".live-qiye-nav li").eq(e.activeIndex).hasClass("house_intro") ? $("#house_intro").show() : $("#house_intro").hide(), $(".live-qiye-nav li").eq(e.activeIndex).hasClass("house_type") ? $("#house_typeInfo").show() : $("#house_typeInfo").hide(), $(".live-qiye-nav li").eq(e.activeIndex).hasClass("house_coupon") ? zbvd.isadmin ? $("#house_couponInfo").show() : $("#house_coupon_appoint").show() : zbvd.isadmin ? $("#house_couponInfo").hide() : $("#house_coupon_appoint").hide(), $("#tab-content-msgwrapper>.swiper-slide").eq(e.activeIndex).hasClass("isfree") ? (zbvd.isfree && (zbvd.isadmin || zbvd.levels), $("#upgradelink").show()) : $("#upgradelink").hide(), setTimeout(function() {
							e.onResize()
						}, 2e3)
					}
				}), $("#banglist-content").length > 0 && (imwebsocket.InitInviteTabList(), $(document).on(ClickOrTap, ".ranking-tab li", function() {
					$(this).addClass("on").siblings().removeClass("on");
					var e = $(this).index(".ranking-tab li");
					bangSwiper.slideTo(e, 300, function() {
						bangSwiper.onResize(), tabSwiper.onResize()
					})
				}).on(ClickOrTap, ".p_rule", function(e) {
					return e.stopPropagation(), $(this).find(".rule-down").toggleClass("rule-up"), $(this).next().toggle(), bangSwiper.onResize(), tabSwiper.onResize(), !1
				})), $(document).on("click", ".live-qiye-nav li:not([nobind])", function() {
				...
				}), $(".c_loading").length < 0 && $("#chat-msglist").before("<div class='c_loading' style='text-align: center;font-size:14px;color: gray;visibility: hidden;'>下拉加载更多</div>"), $("#speakBox").scroll(function(e) {
				...
				var e = "",
					...
					i = "",
					o = !0;
				$(document).on("click", ".liveppt-btn.btn-first", function() {
					null != pptSwiper && pptSwiper.slideTo(0, 300)
				}).on("click", ".liveppt-btn.btn-last", function() {
					null != pptSwiper && pptSwiper.slides.length > 0 && pptSwiper.slideTo(pptSwiper.slides.length - 1, 300)
				}).on("click", ".first-btn.btn-first", function() {
					null != pptSwiper && pptSwiper.slideTo(0, 300)
				}).on("click", ".last-btn.btn-first", function() {
					null != pptSwiper && pptSwiper.slides.length > 0 && pptSwiper.slideTo(pptSwiper.slides.length - 1, 300)
				}).on("click", ".huifu-alert", function(e) {
					...
					$(".huifu-alert").attr("lastmsgids", "").hide(), e.stopPropagation()
				}).on("click", ".huifu-alert-icon-close", function(e) {
					$(".huifu-alert").attr("lastmsgids", "").hide(), e.stopPropagation()
				}).on("click", "li[attr-nickname][data-msgtype='1'][attr-uid!='-1'] .msg-content", function(e) {
					if (!$(this).parent().is(".oupai-dynamic")) {
						...
						if (!utils.IsNullOrEmpty(o)) {
							var n = "";
							n = $(this).hasClass("huifu") ? i + ":" + $(this).find(".msg-content-huifu>span").text() : i + ":" + $.trim($(this).text()), "Vertical" == zbvd.tplname ? ($("#btn_clicksend").attr({
								...
								commentid: t.attr("attr-id"),
								tuserid: t.attr("attr-uid")
							})), $(".speakInput").attr("placeholder", n).data("msgtype", 1), e.stopPropagation()
						}
					}
				}).on("click", "li[attr-nickname][data-msgtype='1'][attr-uid!='-1'] .oupaiMsgContent", function(e) {
					console.log("消息回复");
					var t = $(this).closest("li"),
						i = t.attr("attr-nickname"),
						o = t.attr("attr-id");
				...
				}).on("click", ".backContentBox", function() {
					$("body").removeClass("replymsg"), $(".speakInput").attr("placeholder", "来说点什么吧..."), $(".btnLiveTalk").attr({
						commentid: "",
						tuserid: 0
					})
				}).on("click", ".marry-chat-delete", function(e) {
					$(this);
					speakId = $(this).closest("li").attr("attr-id"), popup.confirm("温馨提示", "确定撤回吗？", ["确定", "取消"], function(e) {
						"ok" == $(e.target).attr("class") && imwebsocket.UnDo(speakId, 1), this.hide()
					}), e.stopPropagation()
				}).on("click", ".unreadMsgTip", function() {
					$(this).attr("firstmsgid");
					1 == zbvd.IsScreen && "2" == zbvd.modeltype && zbvd.ouPaiCustomId.indexOf("," + zbvd.zbid + ",") > -1 ? utils.moveToEndByOuPai($(".liveBoxContent"), function() {}) : utils.moveToEnd($("#speakBox"), function() {}), $(this).find("var").text(0), $(this).removeAttr("firstmsgid"), $(this).hide()
				}).on("click", ".userphoto,.redbag-list>li>img,a.remove-msg,.oupaiName", function(e) {
					function t(e, t) {
						e.removeClass("weui_actionsheet_toggle"), t.removeClass("weui_fade_toggle"), t.on("transitionend", function() {
							t.hide()
					...
					i ? $(".weui_actionsheet_menu[block],.weui_actionsheet_menu[unblock]").show() : $(".weui_actionsheet_menu[block],.weui_actionsheet_menu[unblock]").hide(), i && (0 != zbvd.repayId && zbvd.topicid != $(this).parent("div").parent("li").attr("relayTopicId") ? 1 != o && ($(".weui_actionsheet_menu[block]").find(".weui_actionsheet_cell[type='1']").hide(), $(".weui_actionsheet_menu[unblock]").find(".weui_actionsheet_cell[type='1']").hide()) : ($(".weui_actionsheet_menu[block]").find(".weui_actionsheet_cell[type='1']").show(), $(".weui_actionsheet_menu[unblock]").find(".weui_actionsheet_cell[type='1']").show()));
					var n, s;
					if ($(e.target).is("[from_reward]")) s = $(this).attr("uid"), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[delmsg]").hide(), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[shang]").hide();
					else {
						n = $(this).closest("li"), s = n.attr("attr-uid"), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[shang]").show(), $(".weui_actionsheet_menu[delmsg]").data("msgid", n.attr("attr-id"));
						try {
							$(".weui_actionsheet_menu[topmsg]").data("msgid", n.attr("attr-id")), $(".weui_actionsheet_menu[deltopmsg]").data("msgid", n.attr("attr-id")), 1 == n.data("msgtype") ? ($(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[topmsg]").show(), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[deltopmsg]").hide(), $(".weui_actionsheet_menu[topmsg]").data("msgtype", n.data("msgtype"))) : 43 == n.data("msgtype") ? ($(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[topmsg]").hide(), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[deltopmsg]").show(), $(".weui_actionsheet_menu[deltopmsg]").data("msgtype", n.data("msgtype"))) : ($(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[topmsg]").hide(), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[deltopmsg]").hide())
						} catch (e) {}
						zbvd.roleid > 0 || zbvd.levels > 0 || zbvd.userid > 0 && zbvd.userid == s ? $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[delmsg]").show() : $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[delmsg]").hide(), i && 0 != zbvd.repayId && zbvd.topicid != $(this).parent("div").parent("li").attr("relayTopicId") && (1 != o ? $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[delmsg]").hide() : $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[delmsg]").show(), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[topmsg]").hide(), $(".weui_actionsheet.userphotomenu .weui_actionsheet_menu[deltopmsg]").hide())
					}
					var a = $(this).closest("li").attr("attr-id");
					$(".weui_actionsheet_cell,.complaint").attr("ids", a), $("#actionSheet_wrap").show();
					var d = $("#mask"),
						r = $("#weui_actionsheet");
					r.addClass("weui_actionsheet_toggle"), d.show().focus().addClass("weui_fade_toggle").one("click", function() {
						t(r, d)
					...
					p.data("hostimg", c), p.data("hostuserid", s), p.data("hostusername", l), $("#actionsheet_cancel").one("click", function() {
						$(".weui_actionsheet_menu").removeAttr("uid", s), t(r, d)
					}), d.unbind("transitionend").unbind("webkitTransitionEnd")
				}).on("click", ".unglobalword", function(e) {
					var t = $(this).attr("ids"),
						i = $(this).attr("type");
					$.ajax({
						type: "POST",
						...
						success: function(e) {
							layer.msg(e.Msg), setTimeout(function() {
								$("#actionSheet_wrap").hide(), layer.closeAll()
							}, 1e3)
						}
					}), e.stopPropagation()
				}).on("click", ".setgolbalbackreason", function(e) {
					...
					$.ajax({
						...
						success: function(e) {
							layer.msg(e.Msg), setTimeout(function() {
								layer.closeAll(), $("#actionSheet_wrap").hide()
							}, 1e3)
						}
					}), e.stopPropagation()
					...
					if (1 == i) t = "setuserback";
						...
						if (2 == i) return $(".setgolbalbackreason").attr("uid", e), layer.open({
							title: "选择原因",
							type: 1,
						...
						if (3 == i) {
							...
							n = "1" == a ? "欺诈" : "2" == a ? "色情低俗" : "3" == a ? "政治敏感" : "4" == a ? "违法暴力" : "5" == a ? "血腥残暴" : "6" == a ? "广告骚扰" : "7" == a ? "侮辱谩骂" : "", t = "globallock", s = 0
						}
					}
					...
					var d = $("#mask"),
						r = $("#weui_actionsheet");
					r.addClass("weui_actionsheet_toggle"), $(".weui_actionsheet_menu").removeAttr("uid", e), function(e, t) {
						e.removeClass("weui_actionsheet_toggle"), t.removeClass("weui_fade_toggle"), t.on("transitionend", function() {
							t.hide()
						}).on("webkitTransitionEnd", function() {
							t.hide()
						}), $(".backContentBox").click(), $("#actionSheet_wrap").hide()
					}(r, d)
					...
					var n = $("#mask"),
						s = $("#weui_actionsheet");
					s.addClass("weui_actionsheet_toggle"), $(".weui_actionsheet_menu").removeAttr("uid", e), function(e, t) {
						e.removeClass("weui_actionsheet_toggle"), t.removeClass("weui_fade_toggle"), t.on("transitionend", function() {
							t.hide()
						}).on("webkitTransitionEnd", function() {
							t.hide()
				...
				}).on("click", ".weui_actionsheet_menu[delmsg]", function() {
					try {
						$("#actionsheet_cancel").click()
					} catch (e) {}
					speakId = $(this).data("msgid"), popup.confirm("温馨提示", "确定撤回吗？", ["确定", "取消"], function(e) {
						if ("ok" == $(e.target).attr("class")) {
							imwebsocket.UnDo(speakId, 1);
							var t = $(".chat-msglist li[attr-id='" + speakId + "']");
							t.next(".news-alert-time").remove(), t.prev(".news-alert-time").remove(), t.slideUp("fast", function() {
								t.remove()
							})
						}
						this.hide()
					})
				}).on("click", ".weui_actionsheet_menu[topmsg]", function() {
					try {
						$("#actionsheet_cancel").click()
					} catch (e) {}
					...
					$.when($.post("/liveajax/checkVouchNums", {
						...
						e > -1 && e < 3 ? popup.confirm("温馨提示", "确定推荐消息吗？", ["确定", "取消"], function(e) {
							if ("ok" == $(e.target).attr("class")) {
								var t = $(".chat-msglist li[attr-id='" + speakId + "']"),
									i = {
								...
								o.find(":nth-child(n)").remove(), i.nickname = $.trim(o.text()), imwebsocket.TopMsg(speakId, 43, i);
				...
				}).on("click", ".weui_actionsheet_menu[deltopmsg]", function() {
					try {
						$("#actionsheet_cancel").click()
					} catch (e) {}
					speakId = $(this).data("msgid"), popup.confirm("温馨提示", "确定取消推荐消息吗？", ["确定", "取消"], function(e) {
						if ("ok" == $(e.target).attr("class")) {
							var t = $(".chat-msglist li[attr-id='" + speakId + "']"),
								i = {
									userphoto: t.find(".userphoto").attr("src"),
								...
								o = t.find(".nickname").clone();
							o.find(":nth-child(n)").remove(), i.nickname = $.trim(o.text()), imwebsocket.TopMsg(speakId, 44, i);
							var n = $(".chat-msglist li[attr-id='" + speakId + "']");
							n.slideUp("fast", function() {
								n.remove()
							})
				...
				}).on("click", ".live_otherPay", function() {
					$(".QRcodePay").show()
				}).on("click", ".QRcodePay", function(e) {
					...
					$(".QRcodePay").hide()
				}).on("click", ".news-alert-red-packet.reward>span", function() {
					$(".LmTipsBox").attr("attr-id", $(this).closest("li").attr("attr-id")), $(".live_headpic img,.live_head_pic img").attr("src", $(this).data("imgurl")), $(".thank_money var").text(Number($(this).data("money")) / 100), i = $(this).data("response"), e = $(this).find("var").eq(0).text(), t = $(this).find("var").eq(1).text(), $(".replyresult .r1").text(t), $(".replyresult .r2").text(e);
					var o = $(this).closest("li");
					$(".LmTipsBox .live_towhy").html(e + "<var>赞赏了</var>" + t), o.hasClass("qiangqinbag") && $(".LmTipsBox .live_towhy").html(e + "<var>为抢亲加油！</var>"), "" != i && "null" != i ? ("baobao" == i ? $(".replyresult .r3").text("拥抱") : "loveyou" == i ? $(".replyresult .r3").text("握手") : "meme" == i && $(".replyresult .r3").text("么么哒"), $(".replyresult").show(), $(".thankBox").hide()) : ($(".replyresult").hide(), $(".replyresultNone").show(), "true" == $(this).data("myself") && ($(".LmTipsBox .live_towhy").html(e + "<var>赞赏了</var>你"), $(".thankBox").show(), $(".rgdetail").show())), $(".rgdetail").show(), $(".redbag_rule_2").show(), "true" == $(this).data("myself") && ($(".redbag_rule_2").hide(), $(".redbag_rule_3").show()), $(".thank_money").show(), "" != $(this).attr("data-money") && "null" != $(this).attr("data-money") && void 0 != $(this).attr("data-money") || $(".thank_money").hide(), $(".LmTipsBox").css({
						top: "0",
						display: "block"
					})
				}).on("click", ".rgdetail .rglist", function() {
					$(".managerThankBox").slideToggle(), o ? (o = !1, $(this).parent(".rgdetail").addClass("on")) : (o = !0, $(this).parent(".rgdetail").removeClass("on")), $(".rewrd-content-box").slideToggle()
				}).on("click", ".LmTipsBox", function(e) {
					if ("rglist" == e.target.className) return !1;
					$(this).hide(), $(".managerThankBox").hide(), $(".rgdetail").hide(), $(".replyresultNone").hide(), $(".redbag_rule_3").hide(), o = !0, $(".rgdetail").removeClass("on")
				}).on("click", ".redbag_box .redbag_cancel", function(e) {
					$(".redbag_box").css("top", "999px")
				}).on("click", "#livevote-sendvote .dialog-vote-select-bt", function() {
					...
					imwebsocket.SendMessage("vote", t, "", e, 0, 25), $("#livevote-sendvote").hide()
				}).on("click", "#btnVoterPublish", function() {
					UpdateVote(), $("#divConfirm").toggle(), $("#livevote").toggle(), imwebsocket.SendMessage("vote", _votetitle, "", _voteid + "@" + _datatype + "@" + _votetype_name + "@" + _votetype, 0, 25)
				}).on("click", "#coupon-select-send", function() {
					...
					var e = $("#coupon-select-list :radio[name='RememberMe']:checked").eq(0),
						...
						n = e.attr("_ltype") || "0";
					imwebsocket.SendMessage("coupon", i, o, t, 0, 29, n), e.prop("checked", !1), $("#coupon-select-win").hide()
				}).on("click", "#recommend-product", function() {
					var e = $(this);
				...
				}).on("click", ".btnSetRecPro", function(e) {
					...
					}, "json"), e.stopPropagation()
				}).on("click", "#handle-undo-product", function(e) {
					var t = $(this).data("id");
					"1" == $(this).data("target") ? imwebsocket.SendMessage("", t, "", 0, 0, 39) : imwebsocket.SendMessage("", t, "-1", 0, 0, 39), e.stopPropagation()
				}).on("click", "#getshopcoupon", function(e) {
				...
				}).on("click", ".ver-pick-btn.addcar,.ver-pick-btn.tobuy", function(e) {
					var t = zbvd.loginuser.nickname;
					/^[a-zA-Z0-9_\u4e00-\u9fa5]+$/.test(t) && t.length > 2 && (t = t.substr(0, 1) + "***" + t.substr(-1));
					var i = t;
					$(this).hasClass("tobuy") ? i += "正在购买" : i += "已加入购物车", imwebsocket.SendMessage("", i, "", 0, 0, 1, "SysMsg")
				})
			},
			uid: zbvd.userid,
			rid: zbvd.roleid,
			...
			ReConnect: function() {
				clearInterval(dohand), zbvd.block || (dohand = setInterval(function() {
					(null == wss || 3 == wss.readyState) && reconncount < 8 ? (imwebsocket.ConnectServer(), reconncount += 1) : clearInterval(dohand)
				}, 5e3))
			},
			ProcessMsg: function(msg) {
				if (zbvd.ouPaiCustomId.indexOf("," + zbvd.zbid + ",") > -1 && zbvd.IsScreen && "2" == zbvd.modeltype && (msg.UserInfo && (msg.UserInfo.nickname = utils.CustomStartAndEndSubstr(msg.UserInfo.nickname, 3)), msg.ReplyUser && "" != msg.ReplyUser.nickname && (msg.ReplyUser.nickname = utils.CustomStartAndEndSubstr(msg.ReplyUser.nickname, 3))), msg.UserInfo && "" != msg.UserInfo.headimgurl && (msg.UserInfo.headimgurl = utils.resizeImg(msg.UserInfo.headimgurl, 132, 132)), "欢迎消息" == msg.Types)(window.zbvd && zbvd.isopenonline && zbvd.roleid > 0 || 1 == zbvd.levels || 1006603 == zbvd.zbid) && !utils.IsNullOrEmpty(msg) && "onlineCount" in msg && (msg.onlineCount = msg.onlineCount || 0, "Vertical" == zbvd.tplname ? $(".onlinecount").text("(" + msg.onlineCount + ")") : 4 == zbvd.modeltype ? $("#lbl_onlinecount").text("(" + msg.onlineCount + ")") : $(".onlineuser .onlinecount").text("(" + msg.onlineCount + ")"));
					...
					if ("2" == zbvd.modeltype || "4" == zbvd.modeltype) {
						...
						var nickName = "";
						nickName = void 0 !== msg.todoType && 1 == msg.todoType ? msg.Msg.nickName : msg.Msg, $("#whocomein  span").text(nickName), $("#whocomein").show(), $("#whocomein").addClass("enter"), setTimeout(function() {
							this.tipItem = !1, $("#whocomein").removeClass("enter"), $("#whocomein").hide()
						}, 2500)
					}
				} else if ("提问" === msg.Types) imdebug && console.log("提问"), msgUtils.showNormal(msg);
				else if ("直播红包" == msg.Types) if (15 == msg.Msg.msgtype) {
					"object" != typeof msg.Msg.MsgInfo && (msg.Msg.MsgInfo = JSON.parse(msg.Msg.MsgInfo)), msgUtils.showNormal(msg), zbvd.ouPaiCustomId.indexOf("," + zbvd.zbid + ",") > -1 && "object" == typeof newredbag && newredbag.newredbagContentApp.ouPaiRedBagHandle(msg.Msg.ParentId);
					var msgjson = "GATHERVISITLOGS " + JSON.stringify({
						evtId: 1002,
						refUrl: document.referrer,
						curUrl: document.URL
					});
					wss.send(msgjson), imdebug && console.log(msgjson)
					...
					if (59 == msg.Msg.msgtype) imdebug && console.log("多轮抽奖"), msgUtils.showNormal(msg);
					...
					else if (67 === msg.Msg.msgtype) topicdyapp.pindex = 1, topicdyapp.getinteracts(), topicdyapp.isNewCoi = !0;
					else if (68 === msg.Msg.msgtype) {
						var obj = JSON.parse(msg.Msg.content);
						if ("object" == typeof oupaiCustomApp) return "recommend_procuct" == obj.type ? oupaiCustomApp.recommendProductId = obj.productid : oupaiCustomApp.recommendProductId = 0, void oupaiCustomApp.resetRecommendProduct(function() {
							setTimeout(wxvideoplayer.resizeContentHeight, 1e3)
						})
					} else {
						if (71 == msg.Msg.msgtype) return "object" == typeof oupaiCustomApp && "" != msg.Msg.tnickName && (console.log(msg.Msg.tnickName), oupaiCustomApp.refreshProductSales(JSON.parse(msg.Msg.tnickName))), void msgUtils.showNormal(msg);
					...
					if (0 == msg.todoType) msg.Msg.OnWall ? imwebsocket.delLiveMsgHandle(msg.Msg.Ids) : imwebsocket.delLiveCommentHandle(msg.Msg.Ids);
						...
						if (43 == msg.Msg.msgtype || 44 == msg.Msg.msgtype) {
							...
							del_node.slideUp("fast", function() {
								del_node.remove()
							})
						...
						case 3:
							...
							break;
						case 1:
							zbvd.IsOpenInteractionEffects && startDropAnimate(msg.Msg.content), "Wedding" == zbvd.tplname && startGiftDrop(msg.Msg.content);
						case 2:
						case 5:
							if ("annualmetting" == $("body").data("skin")) {
								msgUtils.showNormal(msg), setQiPaoStyle(qipaoIndex);
								break
							}
							if ("Wedding" == zbvd.tplname) {
								msgUtils.showNormal(msg), setWeddingStyle(weddingIndex);
								break
							}
						case 0:
						case 6:
						...
						case 48:
							...
							break;
						case 25:
							msg.Msg.tnickName = msg.Msg.tnickName.replace(/@/g, '"'), msg.Msg.VoteInfo = eval("(" + msg.Msg.tnickName + ")"), msg.Msg.UserInfo = msg.UserInfo, msg.Msg.msgIds = msg.Msg.Ids, msgUtils.showNormal(msg);
							break;
						case 23:
						...
						case 31:
							...
							if (zbvd.isOpenDm ? ($("#danmukuOpt").addClass("on"), $("#danmukuOpt").next("p").html("禁用弹幕")) : ($("#danmukuOpt").removeClass("on"), $("#danmukuOpt").next("p").html("允许弹幕")), zbvd.isOpenDm) $("#lookdanmuOpt").length > 0 ? $("#lookdanmuOpt").parent().show() : $(".dmswitchopt").length <= 0 && $(".morebutton-wrap").append("<li class='swiper-slide'><a class='dmswitchopt on' id='lookdanmuOpt'></a><p>关闭弹幕</p></li>");
								...
								$("#ckdanmu").hide().find(".dm_switch").prop("checked", !1), $(".speakInput").attr("placeholder", "来说点什么吧...").data("msgtype", 1), $("#lookdanmuOpt").parent().hide()
							}
							break;
						case 39:
							msgUtils.showNormal(msg)
						}
						6 == msg.Msg.msgtype && zbvd.isOpenDm ? (imwebsocket.sendDM(msg.UserInfo.nickname + "赞赏了" + msg.Msg.tnickName + "一个" + parseFloat(msg.Msg.content / 100).toFixed(2) + "元红包"), msg.Msg.userid == zbvd.userid && showIntegral()) : 16 == msg.Msg.msgtype && zbvd.isOpenDm && imwebsocket.sendDM(msg.UserInfo.nickname + msg.Msg.content), 2 != zbvd.modeltype && 4 != zbvd.modeltype || 16 != msg.Msg.msgtype || (zbvd.IsOpenZeroGift = void 0 != msg.Msg.gift.isOpenZeroGiftEffect ? msg.Msg.gift.isOpenZeroGiftEffect : zbvd.IsOpenZeroGift, (msg.Msg.gift.GiftMoney > 0 || zbvd.IsOpenZeroGift) && imwebsocket.ShowGift(msg), msg.Msg.userid == zbvd.userid && showIntegral())
					} else {
						if (48 === msg.Msg.msgtype) msgUtils.showNormal(msg);
							...
							if (29 == msg.Msg.msgtype) {
								...
								"function" == typeof getSocketCouponShow && getSocketCouponShow(msg), $(".youhuiquan").attr("guid", msg.Msg.tnickName).fadeIn(), $(".vertical-coupon-icon").addClass("animation-shake"), couponData = msg.Msg
							}
						}
			...
			ConnectServer: function() {
				...
				function distribution(e) {
					e = e.toString(), "" == e && (e = getRandomUserId(0, 100).toString()), e = e.length <= 2 ? parseInt(e, 10) : e.substr(-2);
					for (var t = m / n, i = m % n, o = 0, s = 0, a = 0; a < n; a++) {
						s = i > 0 ? 1 : 0;
						var d = t + s;
						if (e >= o && e < o + d) {
							_wslink = wslinkList[a], imdebug && console.log("userid=" + e + ",range=" + o + "," + (o + d) + ",wslink=" + wslinkList[a]);
							break
						}
						o += d, --i
					}
				...
				if (!(zbvd.wsbanids.indexOf("," + zbvd.topicid + ",") > -1)) {
					...
					var _wslink;
					if (wslinkList.length <= 0) return console.warn("can't find ws server wslink is null"), !1;
					if ("post" == zbvd.sendmode) {
						var connTpId = zbvd.repayId || zbvd.topicid,
							m = 100,
							n = wslinkList.length;
						distribution(getRandomUserId(0, 100).toString()), "" != zbvd.wstpId && connTpId == zbvd.wstpId && (_wslink = wslinkList[wslinkList.length - 1])
					} else {
						var connTpId = zbvd.repayId || zbvd.topicid,
							lastnum = parseInt(connTpId.toString().substr(-1)),
							_index = Math.ceil((lastnum + 1) / (10 / wslinkList.length)) - 1;
						_index > wslinkList.length && (_index = wslinkList.length), _wslink = wslinkList[_index], "" != zbvd.wstpId && connTpId == zbvd.wstpId && (_wslink = wslinkList[wslinkList.length - 1])
					}
					var _rid = this.rid;
					...
					wss = new WebSocket(strkey), wss.onclose = function() {
						imdebug && console.log("wss  is  closeed"), wss = null, isTalking = !1, imwebsocket.ReConnect()
					}, wss.onopen = function() {
						imdebug && console.log("wss  is  open"), clearInterval(dohand), reconncount = 0, isTalking = !0;
						var e = "GATHERVISITLOGS " + JSON.stringify({
							evtId: 1001,
							refUrl: document.referrer,
							curUrl: document.URL
						});
						wss.send(e), imdebug && console.log(e)
						...
						function processSignalMsg(e) {
							if ("正常消息" == e.Types && 0 != e.todoType) {
								if ("audit" == e.Msg.MsgInfo) {
									...
									console.log(t), $("#chat-msglist").append(t)
								}
								if ((1 == e.Msg.msgtype || 23 == e.Msg.msgtype) && "tempid" in e && "" != e.tempid && $('#chat-msglist>li[tempid="' + e.tempid + '"]').length > 0) {
									...
									i.attr("attr-id", e.Msg.Ids), i.find(".msg-content").text().trim() != e.Msg.content && (zbvd.ouPaiCustomId.indexOf("," + zbvd.zbid + ",") > -1 ? -1 != e.Msg.content.indexOf("正在购买") ? i.find(".msg-content").html(e.Msg.content) : i.find(".msg-content").find("label").html(e.Msg.content) : i.find(".msg-content").html(e.Msg.content))
									...
									if (69 == e.Msg.msgtype) {
										var n = [];
										return "" != e.Msg.content && (n = JSON.parse(e.Msg.content)), void(oupaiCustomApp && oupaiCustomApp.refreshProduct(n))
									}
									imwebsocket.ProcessMsg(e)
								}
								1 == e.Msg.msgtype && "" != e.Msg.content && (window.lastmsg = e.Msg.content), heart.playLike({
									message: {
										likeCnt: 2,
										userId: Math.ceil(1e3 * Math.random()),
										special: !1
							...
							} else imdebug && console.log("其他消息"), imwebsocket.ProcessMsg(e)
						}
						imdebug && console.log(evt.data);
						var msg = evt.data;
						"object" != typeof evt.data && (msg = eval("(" + evt.data + ")")), "[object Array]" == Object.prototype.toString.call(msg) ? (msg.forEach(function(e, t) {
							processSignalMsg(e)
			...
			SideTipProducer: function(e, t) {
				function i(e, t) {
					...
					if ("访客进入" == e.Types) {
						var o;
						o = void 0 !== e.todoType && 1 == e.todoType ? e.Msg.nickName : e.Msg, i.msgType = "观看", i.ctype = 1, i.nickname = o, i.typetx = "观看", i.tpmsg = o + "进入了直播间"
					} else {
			...
			ShowGift: function(e) {
				...
				if (zbvd.ouPaiCustomId.indexOf("," + zbvd.zbid + ",") > -1 && zbvd.IsScreen && "2" == zbvd.modeltype) {
					var s = $("#gift-show-item").html(),
						a = juicer(s, e);
					return $("#gift-show-main").append(a), !1
				}
				if (0 == o.length) {
					if (t) {
					...
					o.animateCss("bounceInLeft", function() {}), o = $("#" + i), clearTimeout(queue_gift[i]), queue_gift[i] = setTimeout(function() {
						o.fadeOut(), $("#" + i + " p").css({
							transition: "none",
					...
					o = $("#" + i), o.is(":hidden") ? (o.show(), n.text(d), clearTimeout(queue_gift[i]), queue_gift[i] = setTimeout(function() {
						o.fadeOut(), $("#" + i + " p").css({
							transition: "none",
							transform: "translate3d(0,0,0)"
						})
			...
			SendMessage: function(e, t, i, o, n, s, a) {
				if (this.islogin()) {
					...
					d = (parseInt(zbvd.roleid) || 0) > 0 ? 1 : 0, "1,2,3,4".indexOf(zbvd.modeltype) > -1 && (d = 1), "SysMsg" != a && "NotInSave" != a || (d = 0);
					...
					if (!zbvd.isaudtcomment || 1 != s && 2 != s && 31 != s || 0 != zbvd.roleid || (c.state = 0), zbvd.block && (c.state = 0), 29 == s && (c.tuserId = i, c.tnickName = o), 25 == s) {
						var l = (zbvd.loginuser.nickname, zbvd.loginuser.headimgurl, zbvd.topicid, zbvd.roleid, zbvd.zbid, o.split("@")),
							...
							u = "",
							m = "";
						null != l && l.length > 0 && (p = l[0], u = l[1], m = l[2], votetype = l[3]), c.tuserId = 0, c.tnickName = "{VoteId:@" + p + "@,DataType:@" + u + "@,VoteType_Name:@" + m + "@,VoteType:@" + votetype + "@}"
					}
					if (0 == c.zbId || 0 == c.TopicId || 0 == c.userId) return void(imdebug && console.log("非法输入"));
					var g = 100;
					null != wss && 3 != wss.readyState || zbvd.block || (imwebsocket.ConnectServer(), g = 2e3), wss && setTimeout(function() {
						function e() {
							var e = 'SENGMSG {Types:"正常消息",Msg:' + JSON.stringify(c) + ',tempid: "' + zbvd.userid + "_" + r + '"}';
							imdebug && console.log(e), "post" == zbvd.sendmode && "" != zbvd.sendurl ? $.post(zbvd.sendurl, {
								msg: e
							}) : wss.send(e)
						}
						var i = {
						...
						if (1 == c.msgtype && "" == c.ParentId) {
							var o = c.content.replace(/(\[[\u2E80-\u9FFF|no|ok]{1,3}\])/gi, function(e, t) {
			...
			UnDo: function(e, t) {
				...
				imdebug && console.log(o), "post" == zbvd.sendmode && "" != zbvd.sendurl ? $.post(zbvd.sendurl, {
					msg: o
				}) : wss.send(o)
			},
			TopMsg: function(e, t, i) {
				...
				imdebug && console.log(s), "post" == zbvd.sendmode && "" != zbvd.sendurl ? $.post(zbvd.sendurl, {
					msg: s
				}) : wss.send(s)
			},
			delLiveMsgHandle: function(e) {
				...
				if ($(".chat-msglist li[attr-id='" + e + "']").length > 0) {
					...
					t.next(".news-alert-time").remove(), t.prev(".news-alert-time").remove(), t.slideUp("normal", function() {
						t.remove()
					})
				}
				$("div[name='divVoteConta'][msgIds='" + e + "']").parent().hide(), $("div[name='divVoteConta'][msgIds='" + e + "']").remove()
			},
			SendFile: function(e) {
				var t = $("#upFile" + e).val();
				...
				try {
					if (void 0 != document.getElementById("upFile" + e).files[0] && null != document.getElementById("upFile" + e).files[0]) {
						if (document.getElementById("upFile" + e).files[0].size > 3145728) return alert("对不起,您发送的文件过大，请重新选择……"), !1
					}
				} catch (e) {}
				var a = new Date,
					d = e + a.getYear() + a.getMonth() + a.getDate() + a.getHours() + a.getMinutes() + a.getSeconds() + a.getMilliseconds();
				if (!o) return alert("上传的文件类型错误!"), !1;
				$.ajaxFileUpload({
			...
			SendImage: function(e) {
				...
				try {
					if (void 0 != document.getElementById("upImg" + e).files[0] && null != document.getElementById("upImg" + e).files[0]) {
						if (document.getElementById("upImg" + e).files[0].size > 3145728) return alert("对不起,您发送的图片过大，请重新选择……"), !1
					}
				} catch (e) {}
				if (!o) return alert("上传的图片类型错误!"), !1;
				var a = new Date,
					d = e + a.getYear() + a.getMonth() + a.getDate() + a.getHours() + a.getMinutes() + a.getSeconds() + a.getMilliseconds();
				$.ajaxFileUpload({
					url: LKconfig.WebURL + "/im/UploadFile?siteid=" + LKconfig.WebSiteId + "&fid=" + d,
			...
			BlockUser: function(e) {
				$(".speakInput").attr("placeholder", e ? e.Msg.content : "您已被禁言！"), -2 != zbvd.levels ? $(".speakInput").attr("disabled", "disabled") : $(".speakInput").attr("placeholder", "您涉嫌违规发言，已被禁言"), utils.IsNullOrEmpty(e) || (zbvd.block = !0, 1 == e.Msg.tuserId && (window.location.href = window.location.href)), $("#incomedraw").parents("li").siblings().hide()
			},
			...
			UnBlockAll: function(e) {
				zbvd.block ? imwebsocket.BlockUser(e) : (utils.IsIOS() ? $(".speakInput").attr("placeholder", "来说点什么吧...") : $(".speakInput").removeAttr("disabled", "disabled").attr("placeholder", "来说点什么吧..."), utils.IsNullOrEmpty(e) || (zbvd.blockall = !1), $(".morebutton").hide(), $("#incomedraw").parents("li").siblings().show()), $("#DisableChat").removeClass("on")
			},
			sendDM: function(e, t) {
				if (!($(".danmuBox").length <= 0)) {
					...
					i.on("webkitAnimationEnd", function(e) {
						$(e.target).remove()
					}), $(".danmuBox").append(i);
					var o = $(".danmuBox .dm:last"),
						n = $("body").width() + o.width(),
						s = $(".videowrap").height() - 35;
					dm_top += 35, dm_top > s && (dm_top = 0);
					var a = 16;
					Math.floor(10 * Math.random()) % 2 == 0 && (a = 20), $("style#dynaicSty").length <= 0 && $("<style id='dynaicSty' type='text/css'></style>").appendTo("body"), $("style#dynaicSty").text("@-webkit-keyframes dmmove{from {left:" + $("body").width() + "px;}to {left:-" + n + "px;}}"), o.css({
				...
				if (isLoading && hasTalkHistory) {
					...
					$(".c_loading").length <= 0 ? $("#chat-msglist").before("<div class='c_loading' style='text-align: center;font-size:14px;color: gray;'>加载中...</div>") : $(".c_loading").text("加载中...").css({
						...
						success: function(o) {
							if ($(".c_loading").text("下拉加载更多").css({
								...
								if ("" != o) {
									...
									"asc" == zbvd.watchmode ? $(o).appendTo($("#chat-msglist")) : setTimeout(function() {
										...
										$("#speakBox").scrollTop(e), ismsgtop ? setTimeout(function() {
									...
									}, 500), "annualmetting" == $("body").data("skin") && setQiPaoStyle(), "Wedding" == zbvd.tplname && setWeddingStyle(), null != tabSwiper && "onResize" in tabSwiper && tabSwiper.onResize(), $("#chat-msglist li").length < 10 && $(".c_loading").css({
										visibility: "hidden"
									})
								} else hasTalkHistory = !1, 0 == $("#chat-msglist li").length ? $(".c_loading").text("还没有评论").css({
									visibility: "visible"
							...
							setTimeout(function() {
								isLoading = !0, "annualmetting" == $("body").data("skin") && setQiPaoStyle(), "Wedding" == zbvd.tplname && setWeddingStyle()
							}, 500)
						},
						error: function() {
							isLoading = !0
			...
			InitInviteTabList: function() {
				bangSwiper = null, bangSwiper = new Swiper("#banglist-content", {
					autoHeight: !0,
					useCSS3Transforms: !0,
					lazyLoading: !0,
					preloadImages: !1,
					onSlideChangeEnd: function(e) {
						$(".ranking-tab li").eq(e.activeIndex).addClass("on").siblings().removeClass("on"), tabSwiper.onResize()
					}
				})
			},
	...
	return imwebsocket
}), define("livecommon/module/gift", ["livecommon/module/utils"], function(e) {
	function t(e, t) {
		"undefined" == typeof WeixinJSBridge ? document.addEventListener ? document.addEventListener("WeixinJSBridgeReady", i, !1) : document.attachEvent && (document.attachEvent("WeixinJSBridgeReady", i), document.attachEvent("onWeixinJSBridgeReady", i)) : i(e, t)
	}
	function i(e, t) {
		WeixinJSBridge.invoke("getBrandWCPayRequest", JSON.parse(e), function(e) {
			"get_brand_wcpay_request:ok" == e.err_msg ? ($(document).minTipsBox({
				...
				o(t)
			}, 500), $("#loadingToast").hide(), redbagbol = !0
		})
	}
	function o(e) {
		$(document).minTipsBox({
			...
			dataType: "json",
			success: function(e) {
				$("#loadingToast").hide(), n = !1, e.isok ? ($("#payqrcode").attr("src", "/live/nt?data=" + e.Msg), $("#QRcodePay").show()) : $(document).minTipsBox({
					tipsContent: e.Msg,
					tipsTime: 1
				})
			},
	...
	var n = !1,
		s = {
			...
			calcPrice: function() {
				...
				$("#fs-gift-total").text(parseFloat(e * t).toFixed(2)), $("#fs-gift-send").addClass("enable")
			},
			BindEvent: function() {
				$(document).on("click", "li[data-giftprice]", function() {
					$("#fs-gift-number").slideDown("fast");
					var e = (parseFloat($(this).data("giftprice")) / 100).toFixed(2);
					$("#fs-gift-total").data({
						price: e,
						giftid: $(this).data("id")
					}), $(this).addClass("on").siblings().removeClass("on"), s.calcPrice()
				}).on("click", ".fsl-shuliang-plus", function() {
					var e = parseInt($("#fs-gift-amount").val()) || 1;
					e += 1;
					$("#fs-gift-total").data("price");
					$("#fs-gift-amount").val(e), s.calcPrice()
				}).on("click", ".fsl-shuliang-reduce", function() {
					var e = parseInt($("#fs-gift-amount").val()) || 1;
					if (!((e -= 1) < 1)) {
						$("#fs-gift-total").data("price");
						$("#fs-gift-amount").val(e), s.calcPrice()
					}
				}).on("input", "#fs-gift-amount", function(e) {
					var t = parseInt($(this).val()) || "";
					$(this).val(t), s.calcPrice()
				}).on("blur", "#fs-gift-amount", function() {
					"" == (parseInt($(this).val()) || "") && $(this).val("1"), s.calcPrice()
				}).on("click", "#fs-gift-send", function(e) {
	...
	return s
}), define("livecommon/module/coupon", ["livecommon/module/utils", "jquery.popBox"], function(e, t) {
	var i = {
		Init: function() {
			this.BindEvent()
		},
		BindEvent: function() {
			...
			$(".couponlist .contentwrap").scroll(function() {
				...
				$(".couponlist .contentwrap").scrollTop() + $(".couponlist .contentwrap").height() >= .9 * i && (e || (e = !0, $.post("/live/GetCouponUserList", {
					...
					if (null != i && i.isok) {
						...
						t.find(".coupon-list").append(o), e = !1
					}
				})))
			}), $(document).on("click", ".content-youhui,.youhuiquan", function() {
				e = !1;
				var t = $(this).attr("guid"),
					o = $(this);
				if (null == t || void 0 == t) return $(".youhuiquan").hide(), void window.location.reload();
				$.ajax({
					type: "POST",
					url: "/live/GetCoupon",
					data: {
					...
					error: function() {
						ispost = !1, $("#loadingToast").hide(), $(document).minTipsBox({
							tipsContent: "Error",
							tipsTime: 1
						})
					}
				})
			}).on("click", ".coupon-open", function() {
				if (zbvd.userid) {
					if (!ispost) {
						ispost = !0, $("#loadingToast").show();
						var e = {
							zbid: zbvd.zbid || 0,
							tpid: zbvd.topicid,
						...
						$.ajax({
							...
							beforeSend: function() {},
							success: function(t) {
								ispost = !1, t && t.isok ? ($("#loadingToast").hide(), $(".opencoupon").hide(), i.ShowRedBagDetail(e.codeid)) : ($("#loadingToast").hide(), "have" == t.code ? i.ShowRedBagDetail(e.codeid) : "over" == t.Msg ? ($(".opencoupon").hide(), i.ShowRedBagDetail(e.codeid)) : i.ShowRedBagDetail(e.codeid))
							},
							error: function() {
								ispost = !1, $("#loadingToast").hide(), $(document).minTipsBox({
									tipsContent: "Error",
			...
			}).on("click", ".lookotherscoupon", function() {
				$(".opencoupon").hide(), i.ShowRedBagDetail($(this).attr("guid"))
			}).on("click", ".bag-close,.couponlist,.bag-mask", function() {
				$(".opencoupon").hide(), $(".couponlist").hide(), $(".info-tips").hide()
			}).on("click", ".info .info-tips", function() {
				window.location.href = "/live/myCouponList"
			}).on("click", ".coupon-enter-icon,.side-tip_cp", function() {
				$(".vzui-raise-modal").removeClass("active"), $("#coupon-box-app").parent().addClass("active"), $(".coupon-mask").show(), couponApp && couponApp.reload()
			}).on("click", ".coupon-mask", function() {
				$("#coupon-box-app").parent().removeClass("active"), $(".coupon-mask").hide()
			})
		},
		ShowRedBagOpen: function(e) {
			isGet = !1;
			for (var t = navigator.userAgent, i = ["Android", "iPhone", "SymbianOS", "Windows Phone", "iPad", "iPod"], o = !0, n = 0; n < i.length; n++) if (t.indexOf(i[n]) > 0) {
				o = !1;
				break
			}
			...
			if (couponData = {
				...
				$(".couponNew").find("[name='money']").text(s.money), $(".couponNew").find("[name='couponUserImg']").attr("src", s.headimgurl), $(".couponNew").find("[name='UserNickname']").text(s.nickname), $(".couponNew").find("[name='description']").text(s.description), $(".couponNew").find("[name='CouponName']").text(s.CouponName), $(".couponNew").find("[name='validDate']").text("有效期至：" + s.validDate);
				var d = "";
				d = "" == s.description || null == s.description ? "暂无使用说明" : s.description, $(".couponDesc").find("[name='descNow']").html(d), 1 == s.isGet ? (1 == s.isUser ? $(".Ubutton").text("已经使用") : $(".Ubutton").text("已经领取"), $(".couponNew").find("[name='payfee']").text(""), $(".Ubutton").removeClass("PayUbutton"), $(".Ubutton").removeClass("GetUbutton"), $(".Ubutton").addClass("GoUbutton")) : ("" != s.payfee && null != s.payfee && void 0 != s.payfee ? ($(".Ubutton").addClass("PayUbutton"), $(".Ubutton").removeClass("GetUbutton"), $(".couponNew").find("[name='payfee']").text(s.payfee)) : ($(".Ubutton").addClass("GetUbutton"), $(".Ubutton").removeClass("PayUbutton"), $(".couponNew").find("[name='payfee']").text("")), $(".Ubutton").removeClass("GoUbutton"), $(".Ubutton").text("立即领取")), 0 == s.LineType || "0" == s.LineType ? $(".UmineCoupon").attr("href", "/live/myverifylistnew?stype=0&state=0") : $(".UmineCoupon").attr("href", "/live/myCouponList"), $(".couponNew").show()
			} else layer.msg(msg.Msg)
		},
		RedBagIsOver: function(e) {
			e = JSON.parse(e.Msg), $(".bag-bagtype").hide(), $(".bag-des").html("手慢了，优惠券已经被领取完了！"), $(".coupon-open").hide(), $(".opencoupon .bag-photo").attr("src", e.headimgurl), $(".opencoupon").attr("guid", e.id).show(), $(".coupon-open").attr("guid", e.id), $(".lookotherscoupon").attr("guid", e.id).show(), $(".opencoupon .bag-username").html(e.nickname)
		},
		ShowRedBagDetail: function(e) {
			$(".opencoupon").hide(), $.ajax({
				type: "POST",
				...
				success: function(t) {
					...
					var i = $(".couponlist"),
						o = JSON.parse(t.Msg);
					i.find(".bag-photo").attr("src", o.headimgurl), i.find(".info-name span").html(o.nickname), i.find(".coupon-title span").html(o.total_amount), i.find(".info-des").html(o.content);
					var n = juicer(document.getElementById("coupon-user-list").innerHTML, {
						dataObj: JSON.parse(t.dataObj)
					});
					i.find(".coupon-list").html(n), i.attr("guid", e), i.show()
				}
			})
		},
	...
	return i
}), define("livecommon/module/inputVoice", ["livecommon/module/utils", "livecommon/module/im"], function(e, t) {
	function i() {
		var e = navigator.userAgent;
		navigator.appVersion;
	...
	function d(e) {
		var t = e;
		if (t.addClass("isReaded"), rememberImReaded(t.parents("li").attr("attr-id")), t.hasClass("isPlaying")) _ = !1, x.pause(), T.removeAttr("src"), clearInterval(I), z = 0, r();
		else {
			_ = !0, r(), t.addClass("isPlaying"), t.addClass("audioloading");
			...
			"" != i && (T.attr("src", i), x.volume = 1, x.play(), clearInterval(I), I = setInterval(function() {
				if (t.hasClass("audioloading") && z < 5) {
					var e = Math.random();
					T.attr("src", i + "?v=" + e), z += 1, x.volume = 1, x.play()
				}
	...
	function c(e) {
		y = !1, clearTimeout(l), wx.uploadVoice({
			localId: e,
			isShowProgressTips: 1,
			success: function(e) {
				e.serverId;
				$(".recordingSending").show(), $.ajax({
					type: "post",
					...
					success: function(e) {
						e && e.isok ? (!
						function(e, i) {
							t.SendMessage("audio", e.msg, "", 0, i, 3), h || o()
						}(e, u), $(".recordingSending").find("b").html("上传成功！"), setTimeout(function() {
							$("#btnCancelRec").trigger("click"), h && $("#btnStartRec").click()
						}, 1e3)) : ($(".recordingSending").find("b").html(e.msg), setTimeout(function() {
							$("#btnCancelRec").click()
						}, 1e3))
					},
					error: function() {
						alert("语音上传失败，请重试！"), $("#btnCancelRec").click()
					}
				})
			}
		}), p && (wx.playVoice({
	...
	var g = !0,
		...
		h = !1,
		f = function() {
			u++, u > 60 ? (console.log("clockstop"), u = 60, $(".second_dd var").text(u), clearTimeout(l), $(".recording_tab_box").hasClass("clickRecording") && $("#btnStopRec").click()) : ($(".second_dd var").text(u), l = setTimeout(f, 1e3))
		},
		b = !0,
		w = !1,
		y = !0,
		...
		T = $("#audioPlayer");
	if (null != x) {
		x.loop = !1, x.addEventListener("canplay", function() {
			imdebug && console.log("audio canplay")
		}, !1);
		var I, z = 0;
		x.addEventListener("canplaythrough", function(e) {
			imdebug && console.log("audio canplaythrough readyState:" + x.readyState + " duration:" + x.duration)
		}, !1), x.addEventListener("stalled", function(e) {
			imdebug && console.log("audio stalled"), 0 < $(".audioloading").length && 1 > x.currentTime && T.attr("src", $(".isPlaying").find("audio").attr("src"))
		}, !1), x.addEventListener("pause", function() {
			imdebug && console.log("audio pause duration:" + x.duration)
		}, !1), x.addEventListener("ended", function() {
		...
		}, !1), x.addEventListener("playing", function() {
			$(".isPlaying").removeClass("audioloading").addClass("loadok"), clearInterval(I), z = 0, imdebug && console.log("audio playing")
		}, !1)
	}
	var C = document.getElementById("btnRecording");
	return C && zbvd.userid > 0 && (C && C.addEventListener("touchstart", function(t) {
		console.log(t.changedTouches[0].clientY), t.preventDefault(), e.isAndroid() || i() || $(document).minTipsBox({
			tipsContent: "请到手机端录音",
			tipsTime: 1
		}), imdebug && console.log("touchstart"), v && (v = !1, $(document).minTipsBox({
			tipsContent: "如果不能按住说话，请切换至点击录音模式",
			tipsTime: 1
		})), g = !0, b && y && wx.startRecord({
			success: function(e) {
				r(), x.pause(), g ? (w = !0, b = !1, $("#btnRecording").addClass("on"), $(".inputbar").addClass("recording"), $(".recording_box .tips_dd").text("松开发送"), u = 0, $(".second_dd var").text(u), l = setTimeout(f, 1e3)) : wx.stopRecord({
					complete: function(e) {
						b = !0
					}
				})
	...
	}), C && C.addEventListener("touchmove", function(e) {
		imdebug && console.log("touchmove"), 0 == $(".clickRecording").length && (e.changedTouches[0].clientY >= m ? ($(".recordingTips_2").hide(), $(".recording_box .tips_dd").text("松开发送")) : ($(".recordingTips_2").show(), $(".recording_box .tips_dd").text("松开取消")))
	}), C && C.addEventListener("touchend", function(e) {
		if (imdebug && console.log("touchend"), 0 == $(".clickRecording").length) if (g = !1, $(".inputbar").removeClass("recording"), $(".recordingTips_2").hide(), $(".recording_box .tips_dd").text("按住说话"), $("#btnRecording").removeClass("on"), $(".second_dd var").text("0"), w) {
			...
			console.log(i), clearTimeout(l), 1 > u ? ($(document).minTipsBox({
				tipsContent: "录音时间太短",
				tipsTime: 1
			}), wx.stopRecord({
				complete: function(e) {}
				...
				success: function(e) {
					t = e.localId, i >= m ? c(t) : $(document).minTipsBox({
						tipsContent: "已取消发送",
						tipsTime: 1
					})
				},
		...
		} else wx.stopRecord(), b = !0
	}), document.addEventListener("touchcancel", function(e) {
		g = !1
	})), wx.ready(function() {
		wx.onVoiceRecordEnd({
			complete: function(e) {
				console.log("autostop"), clearTimeout(l), u = 60, $(".inputbar").removeClass("recording").removeClass("recording_2"), $(".recordingTips_2").hide(), $("#btnRecording").removeClass("on"), w = !1, b = !0, $(".second_dd var").text(u), c(e.localId)
			}
		})
	}), $(".recordingMsg").each(function() {
		...
		BindEvent: function() {
			$(document).on("click", ".btnInputVoice", function(t) {
				-2 == zbvd.levels && $(document).popBox({
				...
				$(".recording_tab_box").toggleClass("clickRecording"), a(e)
			}).on("click", ".recording_tab_box", function(e) {
				e.stopPropagation()
			}).on("click", ".recordingMsg", function() {
				d($(this))
			}).on("click", "#btnStartRec", function(e) {
				clearTimeout(l), u = 0, b && (b = !1, wx.startRecord({
					success: function(e) {
						n(), x.pause(), s(), $(".recording_click .btn_dd").addClass("startRec"), $(".inputbar").addClass("recording_2"), $(".recording_click .tips_dd").text("点击按钮，完成录音"), u = 0, $(".second_dd var").text(u), l = setTimeout(f, 1e3)
					},
					fail: function(e) {
						u = 0, b = !0, s()
					}
				...
				u < 1 ? $(document).minTipsBox({
					...
					success: function(e) {
						console.log("stopRecord"), k = e.localId, s(), $(".recording_click .btn_dd").addClass("stopRec"), $(".recording_click .tips_dd").text(""), h && $("#btnSentRec").click()
					},
					fail: function(e) {
						s(), b = !0, $(".second_dd var").text("0"), $(".inputbar").removeClass("recording_2"), $(".recording_click .tips_dd").text("点击按钮录音"), $(document).minTipsBox({
							tipsContent: "录音时间太短",
							tipsTime: 1
						})
					}
				...
				"" != k && (c(k), $(".inputbar").removeClass("recording_2"), $(".recording_click .tips_dd").text("点击按钮录音")), a(e)
			}).on("click", "#btnCancelRec", function(e) {
				clearTimeout(l), k = "", b = !0, y = !0, u = 0, $(".recordingSending").hide(), $(".recordingSending").find("b").html("语音正在发送..."), $(".second_dd var").text("0"), $(".inputbar").removeClass("recording_2"), $(".recording_click .tips_dd").text("点击按钮录音"), s(), a(e)
			})
		}
	}
}), define("livecommon/module/vote_custom", ["livecommon/module/utils"], function(utils) {
	...
	function jsNaticeCall(e) {
		$(document).minTipsBox({
			...
			dataType: "json",
			success: function(e) {
				$("#loadingToast").hide(), is_pay_post = !1, e.isok ? ($("#payqrcode").attr("src", "/live/nt?data=" + e.Msg), $("#QRcodePay").show()) : $(document).minTipsBox({
					tipsContent: e.Msg,
					tipsTime: 1
				})
			},
			error: function(e, t, i) {
				$(document).minTipsBox({
					tipsContent: "支付失败",
					tipsTime: 1
				}), $("#loadingToast").hide(), is_pay_post = !1
			}
		})
	}
	var is_pay_post = !1,
	...
	var numMap = {
		...
		LiveVote = {
			...
			BindEvent: function() {
				$(document).on("click", ".toupiaobtn,.icon-admin-vote,.inputicon.vote", function() {
					$("#livevote").toggle(), $(".panel-more").hide()
				}).on("click", "#livevote .red-mask-02,#livevote .dialog-close", function() {
					$("#livevote").hide()
				}).on("click", "#livevote-choose .icon-vote-title-close,#livevote-choose .red-mask-02", function() {
					$("#livevote-choose").hide()
				}).on("click", "#livevote-result .icon-vote-title-close,#livevote-result .red-mask-02", function() {
					$("#livevote-result").hide()
				}).on("click", '.vote-send-ul,[data-msgtype="25"]', function(e) {
					e.stopPropagation();
					var vtid = $(this).attr("vtid");
					$(this).hasClass("vote-send-ul") && (vtid = $(this).parent().attr("vtid")), $.post("/liveajax/GetVote", {
						vtid: vtid
					}, function(data) {
						if (data && data.isok) {
							var msg = eval("(" + data.Msg + ")");
							msg.zbid = zbvd.zbid, LiveVote.ShowVoteChoose(msg)
						} else $(document).minTipsBox({
							tipsContent: data.Msg,
							tipsTime: 1
						})
					})
				}).on("click", "#livevote-choose .dialog-vote-select-bt.votebtn", function(e) {
					e.stopPropagation();
				...
				}).on("click", "#btn-sendvote-1", function() {
					$("#livevote").hide(), LiveVote.ShowVoteList()
				}).on("click", "#livevote-sendvote .red-mask-02,#livevote-sendvote .icon-vote-title-close", function() {
					$("#livevote-sendvote").hide()
				}).on("click", "#btn-editvote", function() {
					LiveVote.ShowVoteManage()
				}).on("click", "#votemgr .vote-admin-bt", function() {
					LiveVote.ShowNewVote()
				}).on("click", "#votemgr .icon-vote-title-close", function() {
					$("#votemgr").hide()
				}).on("click", "#voteedit .icon-vote-title-close,#voteedit .red-mask-02", function() {
					LiveVote.IsEdit = !1, LiveVote.EditId = 0, $("#voteedit").hide()
				}).on("click", "#voteedit .a01", function() {
					var e = $("#voteedit").find("ul[copynode]").eq(0).clone(!0);
					e.find(":text").val(""), e.attr("subid", 0), e.removeAttr("copynode"), e.append('<li><a class="vote-edit-delete-select" href="javascript:;"></a></li>');
					var t = $(".voteedititems ul[nodeitem]").length;
					t += 1, t = t.toString().replace(/\d+/, function(e) {
						return 10 == e ? "十" : e > 0 && e < 10 ? numMap[e] : e
					}), e.find("span[nodenum]").text(t), $(".voteedititems").append(e)
				}).on("click", "#voteedit .vote-edit-delete-select", function() {
					...
					if (LiveVote.ResetItemNum(), LiveVote.IsEdit) {
						...
						0 != t ? $(document).popBox({
							...
							btnType: "both",
							confirmFunction: function() {
								$.post("/liveajax/delsVote", i, function(t) {
				...
				}).on("click", "#voteedit .a02", function() {
					...
					if (LiveVote.IsEdit ? (e.each(function(e, o) {
						...
						"" == $.trim(n) && (i = !0), t.push($(o).attr("subid") + "|" + n)
					}), o = {
						vid: LiveVote.EditId,
					...
					$.post("/liveajax/addVote", o, function(e) {
						$(document).minTipsBox({
							tipsContent: e.Msg,
							tipsTime: 1
						}), e && e.isok && ($("#voteedit").hide(), LiveVote.ShowVoteManage())
					})
				}).on("click", "#votemgr a[voteitemid]", function() {
				...
				}).on("click", "#votemgr .vote-del", function(e) {
				...
				}).on("click", "#votemgr .vote-edit", function(e) {
					var t = $(this).closest("a[voteitemid]").attr("voteitemid");
					LiveVote.ShowEditVote(t, !0), e.stopPropagation()
				}).on("click", ".dialog-vote-gift-bt", function() {
					var e = parseInt($(this).data("vtid")) || 0;
					if (0 == (parseInt($("#livevote-choose :radio:checked").val()) || 0)) return void $(document).minTipsBox({
						tipsContent: "请选择一个选项",
						tipsTime: 1
					});
					$(".tuwen-mic-bt").data("voteid", e), $(".voteGiftBox").show()
				}).on("click", ".tuwen-close", function() {
					$(".voteGiftBox").hide()
				}).on("click", ".vote-gift-reduce", function() {
					var e = $(".mic-input-number");
					e.val() > 1 && e.val(e.val() - 1)
				}).on("click", ".vote-gift-add", function() {
					var e = $(".mic-input-number");
					e.val(parseInt(e.val()) + 1)
				}).on("click", ".tuwen-mic-bt", function(e) {
					if (!is_pay_post) {
						var t = {
							liveId: zbvd.zbid,
							topicId: zbvd.topicid,
						...
						$("#loadingToast").show(), is_pay_post = !0, $.ajax({
							type: "POST",
							url: "/live/PayCenter",
							data: t,
							dataType: "JSON",
							...
							error: function() {
								is_pay_post = !1, $(document).minTipsBox({
									tipsContent: "请求失败",
									tipsTime: 1
								}), $("#loadingToast").hide()
							}
			...
			ShowVoteResult: function(e) {
				$("#livevote-choose").hide(), $("#livevote-result").html(juicer($("#livevote-result-tpl").html(), e)).show()
			},
			ShowVoteChoose: function(e) {
				$("#livevote-result").hide(), $("#livevote-choose").html(juicer($("#livevote-choose-tpl").html(), e)).show()
			},
			...
			ShowVoteManage: function() {
				$("#livevote").hide(), $.post("/liveajax/GetVoteList", {
					tpid: zbvd.topicid
				}, function(data) {
					if (data && data.isok) {
						var msg = eval("(" + data.Msg + ")");
			...
			ShowNewVote: function() {
				LiveVote.IsEdit = !1, $("#voteedit").html(juicer($("#voteedit-tpl").html(), {
					SignupList: [{}, {}, {}]
				})).show(), $("#voteedit").show(), LiveVote.BindTimePicker($("#server_time").val())
			},
			IsEdit: !1,
			EditId: 0,
			ShowEditVote: function(_vtid, flag) {
				$.post("/liveajax/GetVote", {
					...
					if (data && data.isok) {
						var msg = eval("(" + data.Msg + ")");
						if (flag) msg.EndTime = msg.EndTime.replace("T", " "), $("#voteedit").html(juicer($("#voteedit-tpl").html(), msg)).show(), LiveVote.ResetItemNum(), LiveVote.IsEdit = !0, LiveVote.EditId = _vtid, LiveVote.BindTimePicker(msg.EndTime);
						...
						case 1:
							...
							break;
						case 0:
							msg.EndTime = msg.EndTime.replace("T", " "), $("#voteedit").html(juicer($("#voteedit-tpl").html(), msg)).show(), LiveVote.ResetItemNum(), LiveVote.IsEdit = !0, LiveVote.EditId = _vtid, LiveVote.BindTimePicker(msg.EndTime)
						}
					} else $(document).minTipsBox({
						tipsContent: data.Msg,
						tipsTime: 1
			...
			ResetItemNum: function() {
				$("#voteedit ul[nodeitem]").each(function(e, t) {
					var i = e + 1;
					$(t).find("span[nodenum]").text(i.toString().replace(/\d+/, function(e) {
						return 10 == e ? "十" : e > 0 && e < 10 ? numMap[e] : e
					}))
				})
			},
			DeleteVote: function(e) {
				...
				$.post("/liveajax/delsVote", t, function(t) {
					$(document).minTipsBox({
						tipsContent: t.Msg,
						tipsTime: 1
	...
	return LiveVote
}), define("livecommon/module/vote", ["livecommon/module/utils"], function(utils) {
	...
	var numMap = {
		...
		LiveVote = {
			...
			BindEvent: function() {
				$(document).on("click", ".toupiaobtn,.icon-admin-vote,.inputicon.vote,.livevote", function() {
					$("#livevote").show(), $(".panel-more").hide()
				}).on("click", "#livevote .red-mask-02,#livevote .dialog-close", function() {
					$("#livevote").hide()
				}).on("click", "#livevote-choose .icon-vote-title-close,#livevote-choose .red-mask-02", function() {
					$("#livevote-choose").hide()
					...
					$("#livevote-result").hide()
				}).on("click", 'div[name="divVoteConta"]', function(e) {
					e.stopPropagation();
					...
					if (2 == datatype) {
						...
						var url = "";
						url = 1 != votetype ? "/live/VoteSmallVodieAndPic?tpid=" + zbvd.topicid + "&voteid=" + voteid : "/live/votetext?tpid=" + zbvd.topicid + "&voteid=" + voteid + "&datatype=1", url += "&v=" + Math.random(), window.location.href = url
					}
				}).on("click", "#livevote-choose .dialog-vote-select-bt", function() {
...
var qqfaceswiper = null,
	...
	shang_swiper = null,
	keyWordArr = [];
define("livecommon/module/inputbox", ["livecommon/module/im", "livecommon/module/gift", "livecommon/module/utils", "livecommon/module/redbag", "livecommon/module/coupon", "livecommon/module/inputVoice", "livecommon/module/vote_custom", "livecommon/module/vote", "livecommon/module/controls"], function(e, t, i, o, n, s, a, d, r) {
	function c(t, i, o, n, s, a) {
		return i = (i || "").toString(), "" == i.trim() ? ($(document).minTipsBox({
			tipsContent: "内容不能为空",
			tipsTime: 1
	...
	function l(e, t, i, o, n, s) {
		if (p) {
			...
			if (location.host.indexOf(".vzan.com") > 0) return $("#loadingToast").show(), jsNaticeCall(a, function(e) {
				p = !0, $("#loadingToast").hide()
			}), !1;
			paymentByUrl(baseURL + "/live/PayCenter", a, function() {
				p = !0
			}, function() {
				p = !0
			})
	...
	var p = !0,
		u = {
			Init: function() {
				this.BindEvent(), o.Init(), t.Init(), 156387 == zbvd.zbid || 86149 == zbvd.zbid || 15368 == zbvd.zbid ? a ? a.Init() : console.log("VoteCustom load fail") : d ? d.Init() : console.log("LiveVote load fail"), n.Init(), zbvd.tplname, "Vertical" != zbvd.tplname && $(".inputbar").length > 0 && s.Init()
			},
			Reset: function(e) {
				$("#tab-content .tab-content-msgchat").addClass("swiper-no-swiping"), $(".morebutton").hide(), $("#pingQqFace").hide(), $("body").removeClass("voiceBottom"), $("#c-gift,#fs-gift-number").hide(), $("li[data-giftprice]").removeClass("on");
			...
			BindEvent: function() {
				...
				$(window).bind("orientationchange", function(e) {
					$(".morebutton").hide(), $("#pingQqFace").hide(), $("#c-gift").hide(), e.preventDefault()
				}), i.IsIOS() && 4 != zbvd.modeltype && 1 == zbvd.IsScreen && $(".inputbox input").attr("disabled", !0), 1 == $.cookie("closelookdm_" + zbvd.topicid) ? (zbvd.lookdm = !1, $("#lookdanmuOpt").removeClass("on"), $("#lookdanmuOpt").next().html("观看弹幕")) : zbvd.lookdm = !0, $(document).on("click", ".inputbar", function(e) {
					if ((zbvd.block || zbvd.blockall) && (parseInt(zbvd.roleid) || 0) <= 0 && 0 == zbvd.levels) return e.stopPropagation(), e.preventDefault(), !1
				}), $(document).on("click", function(e) {
					if ($(e.target).closest(".c-gift-white").length > 0) return void e.stopPropagation();
					u.Reset(!1), $("#fs-inputs").hide(), $("#fs-inputbtns").show()
				}).on("click", "#loginbtn", function() {
					window.location.href = "/live/" + $(this).data("actionname") + "-" + zbvd.topicid + "?types=oauths&skId=" + (new Date).getTime()
				}).on("click", ".inputicon.qqface,#icon-vface", function(e) {
					if ((zbvd.block || zbvd.blockall) && (parseInt(zbvd.roleid) || 0) <= 0) return e.stopPropagation(), e.preventDefault(), !1;
					$("#pingQqFace").toggle(), $(this).hasClass("qqface") || $(".new_bottom_input_ul_js").animate({
						bottom: "0"
					}, 200), $(".morebutton").hide(), $("#c-gift").hide(), null == qqfaceswiper && (qqfaceswiper = new Swiper("#qqface-picker", {
						pagination: ".swiper-pagination.qqface",
						paginationClickable: !0,
						onInit: function() {
							$(".post-qq-face-all span i").off().on("click", function(e) {
								...
								$(".speakInput").val(i + t).trigger("input"), e.stopPropagation()
							})
						},
						onTap: function(e) {}
					})), $("body").removeClass("voiceBottom"), e.stopPropagation()
				}).on("click", "#pingQqFace,.morebutton,.mlbn-present", function(e) {
					$(e.target).is(".inviteGuest,.commentcheck,.introduce,.tianjia-pic") || e.stopPropagation()
				}).on("keydown", ".speakInput", function(e) {
					var t = $(this),
						i = t.val();
					if (i.length > 0 && 8 == e.keyCode && i.lastIndexOf("]") == i.length - 1) return i = i.substring(0, i.lastIndexOf("[")), t.val(i).trigger("input"), !1
				}).on("blur", ".speakInput", function(e) {
					4 == zbvd.modeltype && $("body").animate({
						scrollTop: 0
					}, 200)
				}).on("click", ".inputicon.gift", function(e) {
					setTimeout(function() {
						new Swiper(".gift-swiper", {
							pagination: ".swiper-pagination"
						})
					}), $("#c-gift").toggle(), $("#pingQqFace").hide(), $(".morebutton").hide(), $("body").removeClass("voiceBottom"), e.stopPropagation()
				}).on(ClickOrTap, ".inputicon.iconmore", function(e) {
					(zbvd.block || zbvd.blockall) && (parseInt(zbvd.roleid) || 0) <= 0 && 0 == zbvd.levels && $("#incomedraw").parents("li").siblings().hide(), $(".morebutton").toggle(), null == inputbutton_swiper && (inputbutton_swiper = new Swiper(".swiper-container[inputbutton]", {
						...
						observer: !0,
						observerParents: !0
					})), $("#pingQqFace").hide(), $("#c-gift").hide(), $("body").removeClass("voiceBottom"), $(".morebutton").is(":visible") ? $(".tab-content-msgchat").removeClass("swiper-no-swiping") : $(".tab-content-msgchat").addClass("swiper-no-swiping"), "touchstart" == e.type ? e.preventDefault() : e.stopPropagation()
				}).on("click", ".ios-input-box .ios-input-close", function() {
					return $(".ios-input-box").find(".ios-comment-area").val("").end().hide(), $(".backContentBox").click(), !1
				}).on("click", ".inputbox", function() {
					if (i.IsIOS() && 4 != zbvd.modeltype && 1 == zbvd.IsScreen) {
						if ((zbvd.roleid <= 0 || zbvd.repayId > 0) && zbvd.blockall) return !1;
						zbvd.isOpenDm || zbvd.roleid > 0 && 0 == zbvd.repayId ? $(".ios-input-box .danmu").show() : $(".ios-input-box .danmu").hide(), $(".btnLiveTalk").attr("commentid") && $(".btnLiveTalk").attr("tuserid") ? ($(".ios-input-box .danmu").hide(), $(".ios-input-box .ios-comment-area").attr("placeholder", "请输入回复内容"), $(".ios-input-box .ios-comment-area").attr("msgtype", 1)) : $(".ios-input-box .danmu #iosinputdanmu").prop("checked") && zbvd.isOpenDm ? ($(".ios-input-box .ios-comment-area").attr("msgtype", 23), $(".ios-input-box .ios-comment-area").attr("placeholder", "请输入弹幕")) : $(".ios-input-box .ios-comment-area").attr("placeholder", "请输入评论内容"), zbvd.block || $(".ios-input-box").show().find("textarea").focus()
					}
					if (-2 == zbvd.levels) return $.post("/live/IsZoneUserAppeal", {
						userId: zbvd.userid
					}, null, "json").then(function(e) {
						if (e.isok) return layer.msg("我们已经收到您的申诉,请您耐心等待"), !1;
						$("#UserAppealBox").show()
					}), !1
				}).on("click", ".speakInput", function(e) {
					$(this).trigger("input"), e.stopPropagation()
				}).on("input", ".speakInput", function(e) {
					var t = $(this).val();
					"" != t && "" != t.trim() ? ($(".inputicon.iconmore").hide(), $(".btnLiveTalk").show()) : ($(".inputicon.iconmore").show(), $(".btnLiveTalk").hide())
				}).on("focus", ".speakInput", function(e) {
					if (-2 == zbvd.levels) return $.post("/live/IsZoneUserAppeal", {
						userId: zbvd.userid
					}, null, "json").then(function(e) {
						if (e.isok) return layer.msg("我们已经收到您的申诉,请您耐心等待"), !1;
				...
				}).on("click", ".inputicon.photo,.inputicon.file", function(e) {
					if ((zbvd.blockall || zbvd.block) && zbvd.roleid <= 0) return layer.msg("已禁言，不能发送图片"), !1;
						...
						success: function(e) {
							function t() {
								wx.uploadImage({
									localId: i.localId[o],
									success: function(e) {
										o++, i.serverId.push(e.serverId), $.ajax({
											type: "post",
											url: "/liveajax/UploadImagesByWx",
											data: {
												mediaId: e.serverId,
							...
							var o = 0,
								n = i.localId.length;
							i.serverId = [], t(), $(".inputicon.iconmore").trigger("click")
						}
				...
				}).on("touchstart", ".redbagmask", function(e) {
					$(".sendredbagwin").hide(), e.stopPropagation(), "touchstart" == e.type && e.preventDefault()
				}).on("click", ".inputicon.redpacket,.fahongbao,.icon-fsl-red-packet,#icon_redbag,li.hongbao", function(e) {
					$(".panel-more").hide(), $(".sendredbagwin").show(), $("#send-redpacket").show(), "object" == typeof newredbag && newredbag.redpacketapp.init(), e.stopPropagation(), "touchstart" == e.type && e.preventDefault()
					...
					return !1
				}).on("click", ".btnLiveTalk, .ios-input-submit", function(e) {
					var t = $(".speakInput").data("msgtype"),
						i = $(".btnLiveTalk").attr("commentid") || "",
						o = $(".btnLiveTalk").attr("tuserid") || 0;
					$(e.target).is(".btnLiveTalk") ? ($("#ckdanmu").hide(), c("text", $(".speakInput").val(), i, o, 0, t || 1), $(".speakInput").trigger("input")) : (t = $(".ios-comment-area").data("msgtype"), c("text", $(".ios-comment-area").val(), i, o, 0, t || 1), $(".ios-input-box").find(".ios-comment-area").val("").end().hide()), $(".backContentBox").click(), e.stopPropagation()
				}).on("click", ".qpzb-pl-send", function(e) {
					c("text", $("#fs-shuru").val(), "", 0, 0, 1), e.stopPropagation()
				}).on("click", "#btn_clicksend", function(e) {
					var t = $(".speakInput").data("msgtype"),
						i = $("#btn_clicksend").attr("commentid") || "",
						o = $("#btn_clicksend").attr("tuserid") || 0;
					c("text", $(".talk-input02").val(), i, o, 0, t || 1), $(".talk-input02").val(""), $(".new_bottom_js ul").children().show(), $(".new_bottom_input_ul_js").animate({
						bottom: "-4.9rem"
					}, 200), $("#btn_clicksend").removeAttr("commentid tuserid"), e.stopPropagation()
				}).on("click", "#DisableChat,li.notalk", function(e) {
					var t = $(this),
						i = t.hasClass("on") ? 0 : 1;
					$.post("/liveajax/modifyTopic", {
						...
						e && e.isok && (1 == i ? (t.addClass("on"), e.Msg = "已禁止观众评论") : (t.removeClass("on"), e.Msg = "已允许观众评论")), $(document).minTipsBox({
							tipsContent: e.Msg,
							tipsTime: 1
						})
					}), e.stopPropagation(), "touchstart" == e.type && e.preventDefault()
				}).on("click", "#danmukuOpt", function(t) {
					$(this);
					if (window.isposting) return !1;
					window.isposting = !0, $("#loadingToast").show(), $.when($.post("/liveajax/TopicHandle", {
						tpid: zbvd.topicid,
						type: "danmu"
					})).done(function(t) {
						window.isposting = !1, $("#loadingToast").hide(), t.isok ? e.SendMessage("dmOpt", t.dataObj.val, 0, 0, 0, 31) : layer.msg(t.Msg)
					}), t.stopPropagation(), "touchstart" == t.type && t.preventDefault()
				}).on("click", "#netVedio", function(e) {
					openwrap("video"), e.stopPropagation(), $(".menu-box").hide(), "touchstart" == e.type && e.preventDefault()
				}).on("click", "#barUserManage", function(e) {
					window.location.href = "/live/UserManage?zbId=" + zbvd.zbid + "&tpid=" + zbvd.topicid + "&iszbsite=0&v=" + Math.random()
				}).on("click", "div.m_change_vs", function(t) {
					var i = $("ul.fujian-ul").find(".fujian-select-circle-init").parents(".fujian-ul").find("p.fujian-link-address").text();
					e.SendMessage("text", i, "", 0, 0, 26), t.stopPropagation(), "touchstart" == t.type && t.preventDefault()
				}).on("click", "div.m_recove_vs", function(t) {
					e.SendMessage("text", "VideoEmpty", "", 0, 0, 26), t.stopPropagation(), "touchstart" == t.type && t.preventDefault()
					...
					if (t && 13 == t.keyCode && /(Android)/i.test(window.navigator.userAgent)) {
						...
						c("text", $(".speakInput").val(), o, n, 0, i || 1), $(".backContentBox").click(), $(".speakInput").trigger("input"), t.stopPropagation()
					}
				}).on("click", ".live_headpic", function(e) {
					e.stopPropagation()
				}).on("click", ".live_othermoney", function(e) {
					$(".otherRedmoneyBox").show()
				}).on("click", ".otherRedmoneyBox", function(e) {
					if ("rglist" == e.target.className || "money_count" == e.target.className) return !1;
					$(this).hide()
				}).on("click", ".complaint", function(e) {
					var t = $(this).attr("ids");
				...
				}).on("click", ".speaker_shang a,.btn_ilike,.guest_list .guest_img,.shangzhubo,.RoleShang,li.dashang", function(e) {
					if ($(".menu-box").hide(), i.IsWeiXinChat()) if ($(".reward-anchor-box").css({
						...
						if ($(this).hasClass("RoleShang") || $(this).hasClass("weui_actionsheet_cell") || $(this).hasClass("banglist-photo")) shang_swiper.slideTo(0, 1e3, !1), $(".swiper-button-next, .swiper-button-prev").hide(), $($(".reward-anchor-box .swiper-slide")[0]).addClass("swiper-no-swiping"), $("#shangval").val($(this).data("hostuserid")), $($(".live_headpic img")[0]).attr("src", $(this).data("hostimg")), $($(".live_towho")[0]).attr("shangid", $(this).data("hostuserid")), $($(".live_towho")[0]).attr("parentid", ""), $($(".live_towho")[0]).text($(this).data("hostusername")), $($(".reward-anchor-box .swiper-slide")[0]).show(), $(".rolelist").hide(), $(".reward-anchor-box").show();
						else {
							$(".swiper-button-next, .swiper-button-prev").show(), $($(".reward-anchor-box .swiper-slide")[0]).removeClass("swiper-no-swiping"), $(".rolelist").show(), $($(".reward-anchor-box .swiper-slide")[0]).hide(), $("#shangval").val(0);
							...
							if (t.hasClass("left_bubble")) {
								...
								$(".live_headpic img").attr("src", s), $(".live_towho").attr("shangid", o), $(".live_towho").text(n), $(".live_towho").attr("parentid", ""), $(".reward-anchor-box").show()
							} else if (t.hasClass("comment_dd")) {
								var o = t.attr("attrcreateby"),
									n = t.find(".author_name").html(),
									s = t.find(".avatar img").attr("src");
								$(".live_headpic img").attr("src", s), $(".live_towho").attr("shangid", o), $(".live_towho").attr("parentid", t.attr("attr-id")), $(".live_towho").text(n), $(".reward-anchor-box").show()
							} else if (t.parent().hasClass("guest_list")) {
								var o = t.attr("attrcreateby"),
									n = t.find(".guest_title_1").html(),
									s = t.find(".guest_img img").attr("src");
								$(".live_headpic img").attr("src", s), $(".live_towho").attr("shangid", o), $(".live_towho").attr("parentid", ""), $(".live_towho").text(n), $(".reward-anchor-box").show()
							}
						}
						try {
							$("#actionsheet_cancel").click()
					...
					e.stopPropagation(), "touchstart" == e.type && e.preventDefault()
				}).on("click", ".otherRedmoneyBox .gene_confirm", function(e) {
				...
				}).on("click", ".live_redbaglist li", function(e) {
					...
					l("redpacket", n, i, zbvd.zbid, zbvd.topicid, o), e.stopPropagation(), "touchstart" == e.type && e.preventDefault()
				}).on("click", ".inputicon.endlive,.endlivebtn", function(e) {
					$(".menu-box").hide(), $(document).popBox({
						boxContent: "结束直播话题后不能重新开始，</br>确定结束吗？",
						btnType: "both",
						confirmFunction: function() {
							if (!t) return !1;
							t = !1, $("#loadingToast").show(), $.ajax({
								type: "POST",
								...
								success: function(e) {
									t = !0, $("#loadingToast").hide(), $(document).minTipsBox({
										tipsContent: e.Msg,
										tipsTime: 1
				...
				}).on("click", ".inputicon.commentcheck,li.takecheck", function(e) {
					$("#liveCommentAudt").show(), $(".menu-box").hide(), "touchstart" == e.type && e.preventDefault()
				}).on("click", "#liveCommentAudt .close-butt", function(e) {
					$("#liveCommentAudt").hide(), "touchstart" == e.type && e.preventDefault()
				}).on("click", "#liveCommentAudt #ckAudtComment", function(e) {
				...
				}).on("click", "#liveCommentAudt #ckOpenAnonymity", function(e) {
				...
				}).on("click", ".inputicon.coupon", function() {
					u.Reset(), window.location.href = "/live/PushCoupons-" + zbvd.zbid + "?tpid=" + zbvd.topicid
				}).on("click", "#coupon-select-close", function() {
					$("#coupon-select-win").hide()
				}).on("click", "#lookdanmuOpt", function() {
				...
				}).on("click", ".live-wish-layout .wish-btn", function(e) {
					...
					l("reward", i, o, zbvd.zbid, zbvd.topicid, n), e.stopPropagation(), "touchstart" == e.type && e.preventDefault(), t.fadeOut()
				}).on("click", ".btnShowMiniAppIcon", function(e) {
					...
					if (void 0 == t) {
						...
						$.post("/live/GetMiniQRSrc", o, function(e) {
							if ($("#loadingToast").hide(), e && e.isok) if ("" != e.dataObj.logoimg) {
								...
								i.drawImageByCanvas(344, 344, t, function(e) {
									$(".btnShowMiniAppIcon").data("xcqQRImg", e), r.popupMiniCode(e)
								})
							} else $(".btnShowMiniAppIcon").data("xcqQRImg", e.dataObj.baseimg), r.popupMiniCode(e.dataObj.baseimg);
							else layer.msg("获取失败，请重试")
						}, "json")
					} else r.popupMiniCode(t)
				}).on("click", ".showAuctionQrcode", function(e) {
					r.popupAuctionQrcode()
				}).on("click", ".vzui-button.cart,li.cart, .ver-pick-car", function(e) {
					$(".menu-box").hide(), $(".vzui-raise-modal").removeClass("active"), $("#partial-cart-box").parent().addClass("active"), partialCartApp && partialCartApp.loadMore()
				}).on("click", "#liveCommentAudt #noSendImg", function(e) {
				...
				}).on("click", "#icon-wface", function() {
					if (1 == zbvd.LogoPopType) {
						if (void 0 != data) $(".popup-container .popup-body img").attr("src", data.imgurl), $(".popup-container .popup-body p").html(data.msg), $(".popup-container .popup-head .balanced").html(data.name);
						else {
	...
	return u
}), define("livecommon/module/live", ["livecommon/module/utils", "livecommon/module/controls", "livecommon/module/wxvideoplayer"], function(utils, liveControl, wxvideoplayer) {
	...
	var live = {
		Init: function() {
			...
			}), 69 < $("#detail-intro").height() && ($("#tab-to-detail").show(), $(document).on("click", "#tab-to-detail", function() {
				$("#detail-intro").toggleClass("detail"), tabSwiper && tabSwiper.onResize()
			})), setTimeout(function() {
				live.GetOnlineNumber()
			}, 3e3), live.popupqrcode(), "0" == $("#hidUserIsFocus").val() && live.getwxuinfo(), live.BindEvent()
		},
		...
		getwxuinfo: function() {
			$.cookie("focusQRshow_" + zbvd.zbid), zbvd.zbid
		},
		...
		getAuctionProducts: function() {
			isloading || (isloading = !0, $.ajax({
				...
				success: function(e) {
					isloading = !1, e.isok && laytpl($("#auctionProductTpl").html()).render(e.dataObj || [], function(e) {
		...
		BindEvent: function() {
			live.SetToolmenu();
			if ($(document).on("click", ".wonderMoment dl", function(e) {
				$(this).addClass("on").siblings().removeClass("on");
				...
				if (/^[\d]{2}:[\d]{2}:[\d]{2}$/gi.test(t)) {
					...
					else if (1 == $("#hlsMedia1").length && window.objectPlayer) objectPlayer.setcurrentTime(o, !0), objectPlayer.startPlay("");
					else {
						var n = $("#liveVideo");
			...
			}).on("click", "#ent_focus,[focuswin],.fsl-focus-bt", function(e, t) {
				if (1 == zbvd.IsScreen) {
					if (void 0 != t) $(".popup-container .popup-body img").attr("src", t.imgurl), $(".popup-container .popup-body p").html(t.msg), $(".popup-container .popup-head .balanced").html(t.name);
					else {
			...
			}).on("click", ".icon-refresh,.refresh-page", function(e) {
				e.stopPropagation(), window.location.href = live.updateUrl(window.location.href, "ver")
			}).on("click", ".icon-back-index a", function(e) {
				e.stopPropagation()
			}).on("click", ".auction .btn", function(e) {
				var t = $(this),
					i = t.data("status");
			...
			}).on("click", ".live-qiye-nav li:not([nobind])", function(e) {
				var t = $(this).index(".live-qiye-nav li:not([nobind])"),
					i = $(this).attr("menustate");
				8 == i ? $(".liveppt-btn-wrap").show() : $(".liveppt-btn-wrap").hide(), 22 == i ? $(".inputbar").show() : $(".inputbar,#c-gift").hide(), $(".live-tab-content>li").eq(t).show().siblings().hide(), $(this).addClass("on").siblings().removeClass("on"), 21 != i && ($(this).data("showcar") ? zbvd.isadmin ? $("div.look-buy-bottom-nav-fix").show() : $("#shopCar").show() : ($("#shopCar").hide(), $("div.look-buy-bottom-nav-fix").hide()), $(this).hasClass("house_intro") ? $("#house_intro").show() : $("#house_intro").hide(), $(this).hasClass("house_type") ? $("#house_typeInfo").show() : $("#house_typeInfo").hide(), $(this).hasClass("house_coupon") ? zbvd.isadmin ? $("#house_couponInfo").show() : $("#house_coupon_appoint").show() : zbvd.isadmin ? $("#house_couponInfo").hide() : $("#house_coupon_appoint").hide())
			}).on(ClickOrTap, ".marry-live-nav li:not([nobind])", function() {
				var e = $(this).index(".marry-live-nav li:not([nobind])");
				22 == $(this).attr("menustate") ? $(".inputbar").show() : $(".inputbar,#c-gift").hide(), $(".live-tab-content>li").eq(e).show().siblings().hide(), $(this).addClass("active").siblings().removeClass("active"), $(".live-tab-content .marry-live-04").show()
			}).on("click", ".fixedPopBox-bg", function() {
				$(".fixedPopBox").hide()
			}).on("click", ".liveStart,.startlive", function() {
				if (window.zbvd && 3 == zbvd.customertype) return void $(".fixedPopBox").show();
				if ("2,3,4".indexOf(zbvd.modeltype) > -1) {
					var e = $(this).data("pushurl");
					$(".rtmpBox .rtmp_url").html(e).attr("href", e), $(".rtmpBox").show();
					...
					confirmFunction: function() {
						...
						livestartbol = !1, $(".loadingBox").show(), $.ajax({
							type: "POST",
							...
							dataType: "json",
							success: function(e) {
								$(".loadingBox").hide(), e && e.isok ? window.location.reload(!0) : $(document).minTipsBox({
									tipsContent: e.Msg,
									tipsTime: 1
								}), livestartbol = !0
							},
			...
			}).on("click", "#setremind", function() {
				$(".popup-container .popup-body img").attr("src", $(".popup-container").attr("attr-url")), $(".popup-container .popup-body p").html($(".popup-container").attr("attr-msg")), $(".popup-container .popup-head .balanced").html($(".popup-container").attr("attr-name")), setTimeout(function() {
					$(".popup-container").addClass("popup-show")
				}, 200)
			}).on("change", "#ckdanmu .dm_switch", function() {
				$(this).prop("checked") ? $(".speakInput").attr("placeholder", "请输入弹幕").data("msgtype", 23) : $(".speakInput").attr("placeholder", "来说点什么吧...").data("msgtype", 1)
			}).on("change", ".ios-input-box .danmu #iosinputdanmu", function() {
			...
			}).on("click", "#lottery_process_box .click", function(e) {
				$.cookie("is_show_lottery_" + zbvd.topicid, "0", {
					path: "/",
					domain: "." + location.hostname,
					expires: 30
				...
				$.ajax({
					...
					dataType: "json",
					success: function(e) {
						e.isok && (isSign = !0), $(document).minTipsBox({
							tipsContent: e.Msg,
							tipsTime: 2
						})
					}
				})
			}), $(document).on("click", ".erweimaBox .gene_cancel", function() {
				$(".erweimaBox").hide(), $.cookie("is_show_zbguanzhu_" + zbvd.topicid, "2", {
					path: "/",
					domain: "." + location.hostname,
					expires: 1
				})
			...
			this.saveLiveFocus(0, ""), this.GatherDataLog()
		}
	};
	return live
}), define("livecommon/module/vertical", [], function() {
	return {
		Init: function() {
		...
		BindEvent: function() {
			$(document).on("click", function(e) {
				$("#fs-gift-panel").slideUp("fast"), $(".talk-input02").val(""), $(".new_bottom_js ul").children().show(), $(".new_bottom_input_ul_js").animate({
					bottom: "-4.9rem"
				}), e.stopPropagation()
			}).on("click", ".icon-fsl-pl", function(e) {
				$("#fs-inputbtns").hide(), $("#fs-inputs").show(), $("#fs-shuru").focus(), e.stopPropagation()
			}).on("click", "#fs-shuru", function(e) {
				e.stopPropagation()
			}).on("click", ".icon-fsl-close", function() {
				$("body").toggleClass("fs-clear")
			}).on("click", "#fs-gift-panel", function(e) {
				e.stopPropagation()
			}).on("click", ".icon-fsl-present,#icon_gift", function(e) {
				$("#fs-gift-panel").show(), setTimeout(function() {
					new Swiper(".gift-swiper", {
						pagination: ".swiper-pagination"
					})
...
}), define("livecommon/module/house", [], function(e) {
	var t = {
		Init: function() {
			this.BindEvent()
		},
		BindEvent: function() {
			...
			$(document).ajaxSuccess(function(e, i, o) {
				...
				t.log([e, i, o]), $("#loadingToast").hide()
			}), e.loadHouseList(), e.setHouseIntro($("#hidHouseIntro").val() ? JSON.parse($("#hidHouseIntro").val()) : {}), new imgUpload($(".house_intro_upImg"), {
				onComplete: function(e) {
			...
			}), $(document).on("click", "#house_intro", function() {
				$("#house_intro_address").val($("#house_show_address").html()), $("#house_intro_tel").val($("#house_show_tel").text()), $("#house_intro_info").val($("#house_show_info").html().replace(/<br>/g, "\n"));
				...
				$(".house_intro_imgList").find(".house_intro_img").remove().end().prepend(t), $("#house_intro_opt").show()
			}).on("click", ".house_intro_opt_remove", function() {
				$(this).closest(".house_intro_img").remove();
				var e = "";
				$(".house_intro_imgList .house_intro_img img").each(function(t, i) {
					e += $(i).attr("src") + ","
				}), $("#hide_house_intro_imgs").val(e)
			}).on("click", "#house_intro_cancle", function() {
				$("#house_intro_opt").hide()
			}).on("click", "#house_intro_ok", function() {
			...
			}), e.setHouseCoupon($("#hidHouseCoupon").val() ? JSON.parse($("#hidHouseCoupon").val()) : {}), $(document).on("click", "#house_coupon_opt", function() {
				$("#house_coupon_contact").val($("#house_coupon_show_contact").text()), $("#house_coupon_tel").val($("#house_coupon_show_tel").text()), $("#house_coupon_content").val($("#house_coupon_show_content").html().replace(/<br>/g, "\n")), $("#house_Coupon_info").show()
			}).on("click", "#house_coupon_ok", function() {
			...
			}).on("click", "#house_coupon_cancle", function() {
				$("#house_Coupon_info").hide()
			}), $(document).on("click", "#a_look_appoint", function() {
				$("#look_appoint").show(), e.appoint_pagedata = {
					pi: 1,
					ps: 20,
					loadFlag: !1
				}, e.loadAppointList()
			}), t.nodeScroll($("#appoint_list"), function() {
				e.loadAppointList()
			}), new imgUpload($("#house_type_imgBox"), {
				onComplete: function(e) {
					$("#loadingToast").hide(), e = JSON.parse(e), e && e.isok ? ($("#house_type_imgBox #house_type_img").attr("src", e.thumbimg), $("#house_type_imgBox #house_type_img").data("isUpload", !0), $("#house_type_upText").hide()) : alert(e.msg)
				},
				onChange: function() {
					$("#loadingToast").show()
				},
			...
			}), $(document).on("click", "#add_house_typeInfo", function() {
				$("#house_type_content,#house_type_area,#house_type_price").val(""), $("#house_type_img").attr("src", "/livecontent/livecommon/images/img-upload.png"), $("#house_type_upText").show(), $("#house_type_add").show()
			}).on("click", "#house_type_ok", function() {
			...
			}).on("click", ".house_appoint,.go_appoint", function() {
				$("#house_appoint_name,#house_appoint_phone").val(""), $(this).hasClass("house_appoint") ? e.appoint_id = $(this).data("id") : e.appoint_id = 0, $("#house_appointment").show()
			}).on("click", "#appoint_ok", function() {
			...
			}).on("click", ".icon-house-remove", function() {
				var t = $(this).data("id");
				$(document).popBox({
					boxContent: "确定要删除?",
					btnType: "both",
			...
			$.post("/live/GetHouseList", {
				...
				if (e.isok) {
					for (var t = [], i = 0; i < e.dataObj.length; i++) {
						...
						o.Id = e.dataObj[i].Id, o.IsAppoint = e.dataObj[i].IsAppoint, t.push(o)
					}
					laytpl($("#houseListTpl").html()).render(t, function(e) {
		...
		loadAppointList: function() {
			t.appoint_pagedata.loadFlag || ($("#loadingToast").show(), t.appoint_pagedata.loadFlag = !0, $.post("/live/getAppointList", {
				...
				if (e.dataObj.length <= 0) t.appoint_pagedata.loadFlag = !0;
					...
					laytpl($("#appointListTpl").html()).render(e.dataObj, function(e) {
		...
		setHouseIntro: function(e) {
			if (e) {
				e.Address ? $("#house_show_address").html(e.Address).closest(".kfzb-cell").show() : $("#house_show_address").html(e.Address).closest(".kfzb-cell").hide(), e.Phone ? $("#house_show_tel").html(e.Phone).closest(".kfzb-cell").show() : $("#house_show_tel").html(e.Phone).closest(".kfzb-cell").hide(), e.HousesInfo ? $("#house_show_info").html(e.HousesInfo.replace(/\n/g, "<br />")).show() : $("#house_show_info").html(e.HousesInfo).hide();
				for (var t = "", i = e.ImgUrls ? e.ImgUrls.split(",") : [], o = 0; o < i.length && i[o]; o++) t += '<div class="kazb-lpjs-pic">                            <img src="' + i[o] + '" style="width: 100%;">                            </div>';
				$("#hide_house_intro_imgs").val(e.ImgUrls), $("#div_house_show_imgs").empty().append(t)
			}
		},
		setHouseCoupon: function(e) {
			if (e) {
				var t = 0;
				e.CouponContent ? $("#house_coupon_show_content").html(e.CouponContent.replace(/\n/g, "<br />")).closest("p").show() : ($("#house_coupon_show_content").html(e.CouponContent).closest("p").hide(), t += 1), e.Phone ? $("#house_coupon_show_tel").html(e.Phone).closest("p").show() : (t += 1, $("#house_coupon_show_tel").html(e.Phone).closest("p").hide()), e.Contacts ? $("#house_coupon_show_contact").html(e.Contacts).closest("p").show() : (t += 1, $("#house_coupon_show_contact").html(e.Contacts).closest("p").hide()), 3 == t ? $(".kfzb-youhui-f").hide() : $(".kfzb-youhui-f").show()
			}
		},
	...
	return t
}), define("livecommon/module/imageGallery", ["../module/utils"], function(e) {
	var t = {
		Init: function() {
			e.IsWeiXinChat() ? $(document).on("click", ".tab-content-msgchat img[fullsrc]", function() {
				t.viewImg($(this))
			}) : $(document).on("click", ".tab-content-msgchat img[fullsrc]", function() {
				var e = $(this);
				$.magnificPopup.open({
					items: {
						src: e.attr("fullsrc")
...
var recordReaded = {};
localStorage.getItem("recordReaded") && (recordReaded = JSON.parse(localStorage.recordReaded)), $(function() {
	$.fn.autoHeight = function() {
		function e(e) {
			e.scrollTop = 0, $(e).height() > 20 && (e.style.height = "auto"), $(e).height(e.scrollHeight || 35)
		}
		this.each(function() {
			e(this), $(this).on("keydown", function(e) {
				13 == e.keyCode && e.stopPropagation()
			}), $(this).on("keyup", function(t) {
				t.preventDefault(), e(this), 13 == t.keyCode && t.stopPropagation()
			})
		})
	}
}), $.fn.extend({
...
}), require(["jquery.imgUpload", "jquery.popBox", "livecommon/module/inputbox", "livecommon/module/live", "livecommon/module/im", "livecommon/module/shop", "livecommon/module/redbag", "livecommon/module/coupon", "livecommon/module/vertical", "livecommon/module/house", "livecommon/module/imageGallery", "livecommon/module/wxvideoplayer"], function(e, t, i, o, n, s, a, d, r, c, l, p) {
	$(document).ready(function() {
		!
		function() {
			o.Init(), i.Init(), n.Init(), "2" == zbvd.modeltype && (timingredbag && timingredbag.Init(), p.Init()), "Shop" == zbvd.tplname && s.Init(), "Vertical" == zbvd.tplname && r.Init(), "House" == zbvd.tplname && c.Init(), a.GetNewRedBag(), d.GetNewCoupon(), l.Init()
		}()
	})
}, function() {
	console.log(arguments)
}), define("livecommon/app/main", function() {});